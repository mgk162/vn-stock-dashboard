# =============================================================================
# VN Stock Risk Dashboard — v2.0 (Upgraded)
# Architecture: Scan → Decide → Execute → Manage
# Upgrades: True ATR, adjusted-price warning, valuation (P/E P/B ROE),
#           foreign room, improved regime & Wyckoff, data-quality penalty,
#           score reframe, TP/SL with % change, UX polish
# =============================================================================
...
<style>
  ...
  .card { border:1px solid rgba(120,120,120,.2); border-radius:14px;
  ...
  .pill { display:inline-block; padding:2px 9px; border-radius:999px; font-size:.75rem;
          margin:2px 3px 2px 0; border:1px solid; }
  ...
  .p-purple { background:rgba(130,60,200,.1); color:#7030c0; border-color:rgba(130,60,200,.3);}
  ...
  .warn-box { background:rgba(255,160,0,.08); border-left:3px solid rgba(255,160,0,.5);
              border-radius:0 8px 8px 0; padding:7px 11px; font-size:.80rem; line-height:1.5; margin:6px 0; }
  .ind { display:inline-block; padding:2px 8px; border-radius:5px; font-size:.74rem;
         font-weight:600; margin:1px 3px; border:1px solid; }
  ...
  .sig-card { border:1px solid rgba(120,120,120,.18); border-radius:12px;
  ...
  .disclaimer-box { background:rgba(255,160,0,.06); border:1px solid rgba(255,160,0,.3);
                    border-radius:10px; padding:10px 14px; font-size:.79rem;
                    line-height:1.6; margin:8px 0; }
  .val-card { border:1px solid rgba(120,120,120,.18); border-radius:12px;
              padding:10px 14px; margin-bottom:8px; background:rgba(255,255,255,.02); }
...
# TCBS public API base (no auth needed for basic fundamental data)
...
LANG = {
    "vi": {
        ...
        "disclaimer":    "Công cụ phân tích kỹ thuật. Không phải khuyến nghị đầu tư.",
        ...
        "valuation":     "Định giá cơ bản",
        "foreign":       "Dòng tiền ngoại",
        "score_note":    "Điểm KT (0-100) là chỉ số tổng hợp kỹ thuật — KHÔNG phải xác suất thắng",
        "unadj_warn":    "Giá CHƯA điều chỉnh cổ tức. Return/CAGR/Sharpe có thể bị understate cho cổ phiếu chia cổ tức cao.",
    },
    "en": {
        "title":         "📊 VN Stock Dashboard",
        ...
        "disclaimer":    "Technical analysis tool. Not investment advice.",
        ...
        "score_note":    "TA Score (0-100) is a composite technical indicator — NOT a win probability",
        "unadj_warn":    "Prices are NOT dividend-adjusted. Return/CAGR/Sharpe may be understated for high-dividend stocks.",
    },
}
...
GLOSSARY = {
    "vi": {
        ...
        "Information Ratio": "Hiệu quả lợi nhuận active so với rủi ro active.",
        "Volatility": "Biến động giá. Cao = lên xuống mạnh.",
        "CAGR": "Tốc độ tăng trưởng kép hàng năm thực tế.",
        "Wyckoff": "Phân tích cung-cầu: Accumulation → Markup → Distribution → Markdown.",
        "ATR": "Average True Range chuẩn từ OHLC — đo biến động để tính stop.",
        "P/E": "Giá / Thu nhập mỗi cổ phiếu. P/E thấp hơn TB ngành = tiềm năng rẻ.",
        "P/B": "Giá / Giá trị sổ sách. P/B < 1 nghĩa là mua dưới giá trị tài sản.",
        "ROE": "Lợi nhuận trên vốn chủ sở hữu. Trên 15% là tốt.",
        "Room ngoại": "Giới hạn sở hữu nước ngoài. Kịch trần = cầu ngoại bị chặn.",
    },
    "en": {
        ...
        "ATR": "Average True Range from OHLC — proper volatility measure for stop placement.",
        "P/E": "Price/Earnings. Below sector average may indicate undervaluation.",
        "P/B": "Price/Book. Below 1 means buying below net asset value.",
        "ROE": "Return on Equity. Above 15% is generally considered good.",
        "Foreign Room": "Foreign ownership limit. At cap = foreign demand is blocked.",
    },
}
...
def render_ticker_context_control(ticker: str, prices_df: pd.DataFrame,
                                   location: str = "radar", compact: bool = False):
    ...
    chosen_label = st.selectbox(
        f"{t('ctx_label')} — {ticker}", labels, key=widget_key,
...
def fmt_pct_chg(current, reference):
    """Return formatted % change with sign."""
    if pd.isna(current) or pd.isna(reference) or reference == 0:
        return ""
    chg = current / reference - 1
    sign = "+" if chg >= 0 else ""
    color = "#1D9E75" if chg >= 0 else "#c03030"
    return f"<span style='color:{color};font-size:.78rem'>{sign}{chg:.1%}</span>"
...
def build_price_table(tickers: List[str], start: date, end: date, source: str):
    ...
    for tk in tickers:
        ...
        if hist.empty:
            src_used[tk] = "N/A"; row_counts[tk] = 0
            last_dates[tk] = None; first_dates[tk] = None
            continue
    ...
    if not price_frames:
        return pd.DataFrame(), pd.DataFrame(), {
            "src": src_used, "rows": row_counts,
            "last": last_dates, "first": first_dates}
...
def get_vol_series(volumes: pd.DataFrame, tk: str) -> pd.Series:
    return volumes[tk] if isinstance(volumes, pd.DataFrame) and tk in volumes.columns \
        else pd.Series(dtype=float)


# ─────────────────────────── Valuation data / foreign flow ────────────────────
def _request_json_safe(url: str, params: dict = None, headers: dict = None,
                       timeout: int = 10):
    try:
...
def _pick_first_numeric(record, keys) -> float:
...
def _coerce_ratio(v: float) -> float:
    if pd.isna(v):
        return np.nan
    return v / 100 if v > 1 else v


def _merge_non_nan(base: Dict, extra: Dict) -> Dict:
    out = dict(base)
    for k, v in extra.items():
        if k in {"source", "debug"}:
            continue
        if pd.isna(out.get(k, np.nan)) and pd.notna(v):
            out[k] = v
    return out


def _append_debug(debug_msgs: List[str], msg: str):
    if msg and msg not in debug_msgs:
...
def _records_from_any(obj, depth: int = 0) -> List[Dict]:
    ...
    try:
        if isinstance(obj, pd.DataFrame):
            if obj.empty:
                return []
            return [_normalize_record_keys(r) for r in obj.to_dict(orient="records")]
        if isinstance(obj, pd.Series):
            return [_normalize_record_keys(obj.to_dict())]
        if isinstance(obj, dict):
            base = _normalize_record_keys(obj)
            if base:
                out.append(base)
            for v in obj.values():
                out.extend(_records_from_any(v, depth + 1))
            return out
        if isinstance(obj, (list, tuple)):
            for item in obj:
                out.extend(_records_from_any(item, depth + 1))
            return out
    except Exception:
        return out
    return out
...
def _call_noarg_method(obj, method_name: str):
...
def _collect_vnstock_records(symbol: str) -> Tuple[List[Dict], List[str]]:
    debug_msgs: List[str] = []
    records: List[Dict] = []
    try:
        from vnstock import Vnstock  # type: ignore
    except Exception as e:
        return [], [f"vnstock import: {e}"]

    sources = ["VCI", "TCBS", "MSN", "FMP"]
    object_methods = {
        "company": ["overview", "company_overview", "profile", "details", "shareholders"],
        "finance": ["ratio", "ratios", "financial_ratio", "financial_ratios", "overview"],
        "trading": ["foreign_trade", "foreign_trading", "investor_trading", "overview"],
        "quote": ["overview", "price_board", "intraday", "statistics"],
        "listing": ["overview", "company_overview", "details"],
    }
    for src in sources:
        try:
            stock = Vnstock().stock(symbol=symbol, source=src)
        except Exception as e:
            _append_debug(debug_msgs, f"vnstock {src}: {e}")
            continue
        ...
        for obj_name, methods in object_methods.items():
            ...
            for method_name in methods:
                data = _call_noarg_method(obj, method_name)
                if data is None:
                    continue
                recs = _records_from_any(data)
                if recs:
                    for rec in recs:
                        rec.setdefault("_vnstock_source", src)
                        rec.setdefault("_vnstock_object", obj_name)
                        rec.setdefault("_vnstock_method", method_name)
                    source_records.extend(recs)
        if source_records:
            records.extend(source_records)
            _append_debug(debug_msgs, f"vnstock {src}: {len(source_records)} records")
            break
        _append_debug(debug_msgs, f"vnstock {src}: no records")
    return records, debug_msgs


def _pick_numeric_from_records(records: List[Dict], keys: List[str]) -> float:
    keys = [str(k).lower() for k in keys]
    for rec in records:
        if not isinstance(rec, dict):
            continue
        low = _normalize_record_keys(rec)
        for key in keys:
            if key in low:
                val = _safe_float(low.get(key))
                if pd.notna(val):
                    return val
    return np.nan


def _compute_foreign_net_from_records(records: List[Dict]) -> Tuple[float, float]:
    candidate_pairs = [
        ("buyval", "sellval"),
        ("buyvalue", "sellvalue"),
        ("foreignbuyvalue", "foreignsellvalue"),
        ("foreignbuyvolume", "foreignsellvolume"),
        ("fbuyvalue", "fsellvalue"),
        ("buy", "sell"),
    ]
    # direct aggregated fields first
    net5 = _pick_numeric_from_records(records, ["frnetval5d", "foreignnetvalue5d", "netforeignvalue5d", "fnetval5d"])
    net20 = _pick_numeric_from_records(records, ["frnetval20d", "foreignnetvalue20d", "netforeignvalue20d", "fnetval20d"])
    if pd.notna(net5) or pd.notna(net20):
        return net5, net20
    if not records:
        return np.nan, np.nan
    df = pd.DataFrame([_normalize_record_keys(r) for r in records if isinstance(r, dict)])
    if df.empty:
        return np.nan, np.nan
    for col_b, col_s in candidate_pairs:
        if col_b in df.columns and col_s in df.columns:
            net = pd.to_numeric(df[col_b], errors="coerce") - pd.to_numeric(df[col_s], errors="coerce")
            net = net.dropna()
            if not net.empty:
                return float(net.head(5).sum()), float(net.head(20).sum())
    return np.nan, np.nan


@st.cache_data(show_spinner=False, ttl=3600 * 6)
def fetch_valuation_tcbs(symbol: str) -> Dict:
    """
    Fetch valuation data with a vnstock-first strategy.
    Direct TCBS/VCI HTTP endpoints are retained only as secondary fallbacks,
    because those endpoints may return 404 depending on environment/network.
    """
    symbol = symbol.strip().upper()
    result = _valuation_placeholder(symbol)
    debug_msgs: List[str] = []

    vn_records, vn_debug = _collect_vnstock_records(symbol)
    for msg in vn_debug:
        _append_debug(debug_msgs, msg)
    if vn_records:
        vn_ratio = {
            "pe": _pick_numeric_from_records(vn_records, ["pricetoearning", "pe", "p/e"]),
            "pb": _pick_numeric_from_records(vn_records, ["pricetobook", "pb", "p/b"]),
            "roe": _pick_numeric_from_records(vn_records, ["roe", "returnonequity"]),
            "roa": _pick_numeric_from_records(vn_records, ["roa", "returnonassets"]),
            "eps": _pick_numeric_from_records(vn_records, ["earningpershare", "eps"]),
            "dividend_yield": _pick_numeric_from_records(vn_records, ["dividendyield", "cashdividendyield", "dy"]),
            "source": "vnstock",
        }
        result = _merge_non_nan(result, vn_ratio)
        if any(pd.notna(vn_ratio.get(k)) for k in ["pe", "pb", "roe", "roa", "eps", "dividend_yield"]):
            result["source"] = "vnstock"

    # Optional HTTP fallback, but do not let dead endpoints dominate the UI.
    if all(pd.isna(result.get(k)) for k in ["pe", "pb", "roe", "roa", "eps", "dividend_yield"]):
        data, err = _request_json_safe(f"{TCBS_BASE}/tcanalysis/v1/ticker/{symbol}/financialratio")
        if err:
            _append_debug(debug_msgs, f"TCBS financialratio: {err}")
        else:
            items = data if isinstance(data, list) else data.get("data", []) if isinstance(data, dict) else []
            if isinstance(items, list) and items:
                latest = items[0] if isinstance(items[0], dict) else {}
                tcbs_ratio = {
                    "pe": _pick_first_numeric(latest, ["priceToEarning", "pe", "pE"]),
                    "pb": _pick_first_numeric(latest, ["priceToBook", "pb", "pB"]),
                    "roe": _pick_first_numeric(latest, ["roe", "returnOnEquity"]),
                    "roa": _pick_first_numeric(latest, ["roa", "returnOnAssets"]),
                    "eps": _pick_first_numeric(latest, ["earningPerShare", "eps"]),
                    "dividend_yield": _pick_first_numeric(latest, ["dividendYield", "cashDividendYield"]),
                    "source": "TCBS financialratio",
                }
                result = _merge_non_nan(result, tcbs_ratio)
                if any(pd.notna(tcbs_ratio.get(k)) for k in ["pe", "pb", "roe", "roa", "eps", "dividend_yield"]):
                    result["source"] = "TCBS financialratio"
        overview, err = _request_json_safe(f"{TCBS_BASE}/tcanalysis/v1/ticker/{symbol}/overview")
        if err:
            _append_debug(debug_msgs, f"TCBS overview: {err}")
        elif isinstance(overview, dict):
            tcbs_overview = {
                "pe": _pick_first_numeric(overview, ["pe", "priceToEarning"]),
                "pb": _pick_first_numeric(overview, ["pb", "priceToBook"]),
                "roe": _pick_first_numeric(overview, ["roe", "returnOnEquity"]),
                "roa": _pick_first_numeric(overview, ["roa", "returnOnAssets"]),
                "eps": _pick_first_numeric(overview, ["eps", "earningPerShare"]),
                "dividend_yield": _pick_first_numeric(overview, ["dividendYield", "cashDividendYield"]),
                "source": "TCBS overview",
            }
            result = _merge_non_nan(result, tcbs_overview)
            if result.get("source") == "N/A" and any(pd.notna(tcbs_overview.get(k)) for k in ["pe", "pb", "roe", "roa", "eps", "dividend_yield"]):
                result["source"] = "TCBS overview"

    if pd.notna(result.get("dividend_yield")):
        result["dividend_yield"] = _coerce_ratio(result["dividend_yield"])

    has_any = any(pd.notna(result.get(k)) for k in ["pe", "pb", "roe", "roa", "eps", "dividend_yield"])
    if not has_any:
        result["source"] = "N/A"
    result["debug"] = " | ".join(debug_msgs[:6]) if debug_msgs else ""
    return result
...
def _valuation_placeholder(symbol: str) -> Dict:
    return {"pe": np.nan, "pb": np.nan, "roe": np.nan,
            "roa": np.nan, "eps": np.nan, "dividend_yield": np.nan,
            "source": "N/A", "debug": ""}


def valuation_html(val: Dict, lang_code: str = "vi") -> str:
    """Render a compact valuation card without dumping raw endpoint errors into the UI."""
    if not val or all(pd.isna(v) for k, v in val.items() if k not in {"source", "debug"}):
        msg = "Tạm chưa lấy được dữ liệu định giá" if lang_code == "vi" else "Valuation data is temporarily unavailable"
        src = escape(str(val.get("source", "N/A"))) if isinstance(val, dict) else "N/A"
        return f"<div class='val-card'><small>{msg}</small><div style='margin-top:4px'><small style='opacity:.45'>Source: {src}</small></div></div>"

    def row(label, value, note=""):
        val_str = fmt_num(value) if pd.notna(value) else "N/A"
        note_str = f" <small style='opacity:.55'>{note}</small>" if note else ""
        return f"<div class='tbl-row'><span class='tbl-label'>{label}</span><span class='tbl-val'>{val_str}{note_str}</span></div>"

    pe = val.get("pe", np.nan)
    pb = val.get("pb", np.nan)
    roe = val.get("roe", np.nan)
    roa = val.get("roa", np.nan)
    dy = val.get("dividend_yield", np.nan)

    pe_note = ""
    if pd.notna(pe):
        if pe < 0:
            pe_note = "⚠️ EPS âm" if lang_code == "vi" else "⚠️ Negative EPS"
        elif pe < 10:
            pe_note = "🟢 Rẻ" if lang_code == "vi" else "🟢 Cheap"
        elif pe > 30:
            pe_note = "🔴 Đắt" if lang_code == "vi" else "🔴 Expensive"
    pb_note = ""
    if pd.notna(pb):
        if pb < 1:
            pb_note = "🟢 Dưới NAV" if lang_code == "vi" else "🟢 Below NAV"
        elif pb > 4:
            pb_note = "🔴 Cao" if lang_code == "vi" else "🔴 High"
    roe_note = ""
    if pd.notna(roe):
        if roe >= 20:
            roe_note = "🟢 Xuất sắc" if lang_code == "vi" else "🟢 Excellent"
        elif roe >= 15:
            roe_note = "🟡 Tốt" if lang_code == "vi" else "🟡 Good"
        elif roe < 8:
            roe_note = "🔴 Yếu" if lang_code == "vi" else "🔴 Weak"

    src = val.get("source", "N/A")
    dy_str = fmt_pct(dy) if pd.notna(dy) else "N/A"

    return (f"<div class='val-card'>"
            + row("P/E", pe, pe_note)
            + row("P/B", pb, pb_note)
            + row("ROE", roe, roe_note)
            + row("ROA", roa)
            + (f"<div class='tbl-row'><span class='tbl-label'>Dividend Yield</span>"
               f"<span class='tbl-val'>{dy_str}</span></div>")
            + f"<div style='margin-top:4px'><small style='opacity:.45'>Source: {escape(str(src))}</small></div>"
            + "</div>")


@st.cache_data(show_spinner=False, ttl=3600 * 2)
def fetch_foreign_data(symbol: str) -> Dict:
    """
    Fetch foreign room / ownership / recent net buy-sell with a vnstock-first strategy.
    """
    symbol = symbol.strip().upper()
    result = {"room_pct": np.nan, "room_used_pct": np.nan,
              "net_5d": np.nan, "net_20d": np.nan,
              "source": "N/A", "debug": ""}
    debug_msgs: List[str] = []
    ...
    for msg in vn_debug:
        _append_debug(debug_msgs, msg)
    if vn_records:
        room = _pick_numeric_from_records(vn_records, ["foreignpercent", "foreignroom", "foreignownershiprate", "roomforeign", "froom"])
        room_used = _pick_numeric_from_records(vn_records, ["foreignholdingpercent", "foreignownedpercent", "foreigncurrentpercent", "foreignownedrate"])
        net5, net20 = _compute_foreign_net_from_records(vn_records)
        if pd.notna(room):
            result["room_pct"] = _coerce_ratio(room)
        if pd.notna(room_used):
            result["room_used_pct"] = _coerce_ratio(room_used)
        if pd.notna(net5):
            result["net_5d"] = net5
        if pd.notna(net20):
            result["net_20d"] = net20
        if any(pd.notna(result.get(k)) for k in ["room_pct", "room_used_pct", "net_5d", "net_20d"]):
            result["source"] = "vnstock"

    if all(pd.isna(result.get(k)) for k in ["room_pct", "room_used_pct", "net_5d", "net_20d"]):
        overview, err = _request_json_safe(f"{TCBS_BASE}/tcanalysis/v1/ticker/{symbol}/overview")
        if err:
            _append_debug(debug_msgs, f"TCBS overview: {err}")
        elif isinstance(overview, dict):
            room = _pick_first_numeric(overview, ["foreignPercent", "foreignRoom", "foreignOwnershipRate"])
            room_used = _pick_first_numeric(overview, ["foreignHoldingPercent", "foreignOwnedPercent", "foreignCurrentPercent"])
            if pd.notna(room):
                result["room_pct"] = _coerce_ratio(room)
            if pd.notna(room_used):
                result["room_used_pct"] = _coerce_ratio(room_used)
            if any(pd.notna(result.get(k)) for k in ["room_pct", "room_used_pct"]):
                result["source"] = "TCBS overview"

        trade_data, err = _request_json_safe(
            f"{TCBS_BASE}/stock-insight/v1/stock/foreign-trade/ticker/{symbol}",
            params={"page": 0, "size": 20})
        if err:
            _append_debug(debug_msgs, f"TCBS foreign-trade: {err}")
        else:
            items = trade_data.get("data", trade_data) if isinstance(trade_data, dict) else trade_data
            if isinstance(items, list) and items:
                recs = [_normalize_record_keys(x) for x in items if isinstance(x, dict)]
                net5, net20 = _compute_foreign_net_from_records(recs)
                if pd.notna(net5):
                    result["net_5d"] = net5
                if pd.notna(net20):
                    result["net_20d"] = net20
                if result.get("source") == "N/A" and any(pd.notna(result.get(k)) for k in ["net_5d", "net_20d"]):
                    result["source"] = "TCBS foreign-trade"
...
def foreign_html(fd: Dict, lang_code: str = "vi") -> str:
    """Render foreign flow card without raw endpoint-error spam."""
    if not fd:
        return ""
    room = fd.get("room_pct", np.nan)
    room_used = fd.get("room_used_pct", np.nan)
    net5 = fd.get("net_5d", np.nan)
    net20 = fd.get("net_20d", np.nan)
    src = fd.get("source", "N/A")

    room_str = fmt_pct(room) if pd.notna(room) else "N/A"
    room_used_str = fmt_pct(room_used) if pd.notna(room_used) else "N/A"
    room_warn = ""
    if pd.notna(room):
        if room >= 0.49:
            room_warn = (" 🔴 " + ("Gần kịch trần — cầu ngoại bị hạn chế"
                                    if lang_code == "vi" else "Near cap — foreign demand limited"))
        elif room >= 0.45:
            room_warn = " 🟡 " + ("Còn ít room" if lang_code == "vi" else "Low remaining room")
        else:
            room_warn = " 🟢 " + ("Room còn nhiều" if lang_code == "vi" else "Room available")

    net5_str = f"{net5/1e9:.1f}B" if pd.notna(net5) else "N/A"
    net20_str = f"{net20/1e9:.1f}B" if pd.notna(net20) else "N/A"
    net5_c = "#1D9E75" if pd.notna(net5) and net5 > 0 else ("#c03030" if pd.notna(net5) else "#888")
    net20_c = "#1D9E75" if pd.notna(net20) and net20 > 0 else ("#c03030" if pd.notna(net20) else "#888")

    lbl_room = "Room ngoại còn lại" if lang_code == "vi" else "Remaining foreign room"
    lbl_room_used = "Tỷ lệ sở hữu ngoại" if lang_code == "vi" else "Foreign ownership"
    lbl_net5 = "Mua/bán ròng 5P" if lang_code == "vi" else "Net buy/sell 5D"
    lbl_net20 = "Mua/bán ròng 20P" if lang_code == "vi" else "Net buy/sell 20D"
    empty_msg = "Tạm chưa lấy được dữ liệu dòng tiền ngoại" if lang_code == "vi" else "Foreign-flow data is temporarily unavailable"

    if all(pd.isna(fd.get(k)) for k in ["room_pct", "room_used_pct", "net_5d", "net_20d"]):
        return (f"<div class='val-card'><small>{empty_msg}</small>"
                f"<div style='margin-top:4px'><small style='opacity:.45'>Source: {escape(str(src))}</small></div></div>")

    return (f"<div class='val-card'>"
            f"<div class='tbl-row'><span class='tbl-label'>{lbl_room}</span>"
            f"<span class='tbl-val'>{room_str}{room_warn}</span></div>"
            f"<div class='tbl-row'><span class='tbl-label'>{lbl_room_used}</span>"
            f"<span class='tbl-val'>{room_used_str}</span></div>"
            f"<div class='tbl-row'><span class='tbl-label'>{lbl_net5}</span>"
            f"<span class='tbl-val' style='color:{net5_c}'>{net5_str}</span></div>"
            f"<div class='tbl-row'><span class='tbl-label'>{lbl_net20}</span>"
            f"<span class='tbl-val' style='color:{net20_c}'>{net20_str}</span></div>"
            f"<div style='margin-top:4px'><small style='opacity:.45'>Source: {escape(str(src))}</small></div>"
            f"</div>")
...
def _normalize_universe_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rename_map = {
        "ticker": "symbol", "code": "symbol", "board": "exchange",
        "exchangename": "exchange", "exchange_name": "exchange",
        "comgroupcode": "exchange", "stocktype": "type",
        "securitytype": "type", "organname": "organ_name",
        "organshortname": "organ_short_name", "companyname": "organ_name",
        "company_name": "organ_name",
    }
    out = out.rename(columns={k: v for k, v in rename_map.items() if k in out.columns})
    if "symbol" not in out.columns:
        return pd.DataFrame(columns=["symbol", "exchange", "type"])
    if "exchange" not in out.columns: out["exchange"] = ""
    if "type" not in out.columns: out["type"] = ""
    out["symbol"] = out["symbol"].astype(str).str.upper().str.strip()
    out["exchange"] = out["exchange"].astype(str).str.upper().str.strip()
    out["type"] = out["type"].astype(str).str.upper().str.strip()
    exchange_map = {"HSX": "HOSE", "HOSE": "HOSE", "HNX": "HNX",
    ...
    keep_cols = [c for c in ["symbol", "exchange", "type", "organ_name", "organ_short_name"]
                 if c in out.columns]
    return out[keep_cols].drop_duplicates(subset=["symbol"]).reset_index(drop=True) \
               .sort_values([c for c in ["exchange", "symbol"] if c in out.columns])


def _request_json(url: str, params: dict = None):
...
def _fetch_universe_from_vietcap() -> pd.DataFrame:
    urls = [f"{VCI_BASE_URL}/price/symbols/getAll",
...
def fetch_market_universe() -> pd.DataFrame:
    ...
    raise RuntimeError("Cannot fetch market universe. " + " | ".join(errors))
...
def fetch_symbols_by_group(group: str) -> List[str]:
    ...
    try:
        data = _request_json(f"{VCI_BASE_URL}/price/symbols/getByGroup",
                             params={"group": group})
        df = _normalize_universe_df(pd.DataFrame(data))
        if not df.empty:
        ...
        pass
    if group == "VN30":
        return resolve_scan_universe("HOSE")[:30]
    if group == "VN100":
        return resolve_scan_universe("HOSE")[:100]
    return []
...
def resolve_scan_universe(choice: str) -> List[str]:
    ...
    if choice == "HOSE":   filt = df["exchange"].eq("HOSE")
    elif choice == "HNX":  filt = df["exchange"].eq("HNX")
    elif choice == "UPCOM": filt = df["exchange"].eq("UPCOM")
    else:                   filt = pd.Series(True, index=df.index)
...
def build_scan_snapshot_rows(symbols, benchmark, start_date, end_date,
                              data_source, rf_annual, alpha_conf,
                              progress_bar=None, status_slot=None) -> pd.DataFrame:
    symbols = [str(s).upper().strip() for s in symbols if str(s).strip()]
    if not symbols: return pd.DataFrame()
    rows = []
    batches = chunk_list(symbols, MAX_SCAN_SYMBOLS)
    total = len(batches)
    for i, batch in enumerate(batches, start=1):
        ...
        try:
            ...
            continue
        if prices_raw.empty: continue
        asset_cols = [c for c in batch if c in prices_raw.columns]
        if not asset_cols: continue
...
def liq_flag(v) -> str:
    lbl = liq_label(v)
    if lbl in ["Rất tốt", "Excellent", "Tốt", "Good"]:
        return _pm("Giao dịch ổn", "Tradable")
    if lbl in ["Trung bình", "Average"]:
        return _pm("Cẩn thận", "Watch size")
    if lbl in ["Yếu", "Weak"]:
        return _pm("Chỉ mở nhỏ", "Small size only")
...
def liq_metrics(price_s: pd.Series, vol_s: pd.Series) -> Dict:
    ...
    if df.empty:
        return {"avg_vol_20d": np.nan, "avg_val_20d": np.nan,
                "liq_label": "N/A", "liq_flag": "N/A"}
    tv = df["p"] * df["v"]
    avg_v = float(tv.mean())
...
def calc_bb(p: pd.Series, w=20, n_std=2.0):
    ...
    std = p.rolling(w).std(ddof=1)
    return ((mid + n_std * std).reindex(p.index),
            mid.reindex(p.index),
            (mid - n_std * std).reindex(p.index))


def true_range_series(ohlcv: pd.DataFrame) -> pd.Series:
    """
    Compute True Range from OHLCV DataFrame.
    TR = max(High-Low, |High-PrevClose|, |Low-PrevClose|)
    Falls back to |close.diff()| if OHLC not available.
    """
    if ohlcv is not None and not ohlcv.empty and \
       all(c in ohlcv.columns for c in ["high", "low", "close"]):
        h = ohlcv["high"]
        l = ohlcv["low"]
        c = ohlcv["close"]
        pc = c.shift(1)
...
def atr_from_ohlcv(ohlcv: pd.DataFrame, w=14) -> float:
    """True ATR from OHLC data. Falls back to close-diff approximation."""
    tr = true_range_series(ohlcv)
    if len(tr) >= w:
        atr_s = tr.ewm(com=w-1, min_periods=w).mean()
        v = float(atr_s.dropna().iloc[-1]) if not atr_s.dropna().empty else np.nan
        if pd.notna(v): return v
    # Fallback: use close series approximation
    if ohlcv is not None and not ohlcv.empty and "close" in ohlcv.columns:
        tr2 = ohlcv["close"].diff().abs()
        v2 = tr2.ewm(com=w-1, min_periods=w).mean()
        if not v2.dropna().empty:
            return float(v2.dropna().iloc[-1])
    return np.nan


def atr_approx(p: pd.Series, w=14) -> float:
    """Fallback ATR from close-only series."""
    p = p.dropna()
...
def compute_timing(price_s: pd.Series, vol_s: pd.Series = None) -> Dict:
    ...
    if pd.notna(mom63):
        checks.append((True, mom63 > 0.03,
            ...
            _pm(f"Momentum {mom63:.1%} 📉", f"Momentum {mom63:.1%} 📉")))
    checks.append((True, vol_ok, _pm("Biến động thấp ✅", "Volatility low ✅"),
                   _pm("Biến động cao ⚠️", "Volatility high ⚠️")))
    ...
    return {"px": px, "ma20": ma20, "ma50": ma50, "ma200": ma200,
            ...
            "max_score": max_s, "overall": overall, "cls": cls,
            "timing_score": timing_score}


# ─────────────────────────── Regime detection (improved) ──────────────────────
def detect_regime(price_s: pd.Series, bench_s: pd.Series = None) -> Dict:
    """
    Improved regime detection using:
    - Trend alignment (MA50, MA200)
    - Momentum breadth (3M, 6M returns)
    ...
    p = price_s.dropna()
    if len(p) < 40:
        return {"regime": _pm("Chưa đủ dữ liệu", "Insufficient data"),
                "score": np.nan, "vol_state": "unknown", "dd_now": np.nan,
                "breadth_score": np.nan}
    px = p.iloc[-1]
    ma20  = p.rolling(20).mean().iloc[-1]  if len(p) >= 20  else np.nan
    ma50  = p.rolling(50).mean().iloc[-1]  if len(p) >= 50  else np.nan
    ma200 = p.rolling(200).mean().iloc[-1] if len(p) >= 200 else np.nan
    ret21 = p.pct_change(21).iloc[-1]  if len(p) >= 22 else np.nan
    ret63 = p.pct_change(63).iloc[-1]  if len(p) >= 64 else np.nan
    ret126= p.pct_change(126).iloc[-1] if len(p) >= 127 else np.nan
    lr = np.log(p / p.shift(1)).dropna()
    cv  = lr.tail(20).std() * np.sqrt(TDAYS) if len(lr) >= 20 else np.nan
    hv  = lr.std() * np.sqrt(TDAYS) if len(lr) >= 20 else np.nan
    vol_ratio = cv / hv if pd.notna(cv) and pd.notna(hv) and hv > 0 else np.nan
    vol_state = ("high" if pd.notna(vol_ratio) and vol_ratio > 1.25
                 else "elevated" if pd.notna(vol_ratio) and vol_ratio > 1.05
                 else "normal")
    dd_now = float((p / p.cummax() - 1).iloc[-1]) if len(p) > 1 else np.nan

    # Higher highs / lower lows structure (last 60 days)
    if len(p) >= 60:
        p60 = p.tail(60)
        mid = len(p60) // 2
        first_half_high = p60.iloc[:mid].max()
        second_half_high = p60.iloc[mid:].max()
        first_half_low  = p60.iloc[:mid].min()
        second_half_low  = p60.iloc[mid:].min()
    ...
    if bench_s is not None and not bench_s.empty:
        df_rs = pd.concat([p.pct_change(63).rename("s"),
                           bench_s.pct_change(63).rename("b")], axis=1).dropna()
        if not df_rs.empty:
            diff = float(df_rs["s"].iloc[-1]) - float(df_rs["b"].iloc[-1])
            rs_score = clamp(50 + diff * 250)

    # Score components
    trend_pts = sum([
        int(pd.notna(ma20)  and px > ma20)  * 15,
        int(pd.notna(ma50)  and px > ma50)  * 25,
        int(pd.notna(ma200) and px > ma200) * 25,
        int(pd.notna(ma50)  and pd.notna(ma200) and ma50 > ma200) * 15,
    ])
    mom_pts = sum([
        int(pd.notna(ret21)  and ret21 > 0.02)  * 10,
        int(pd.notna(ret63)  and ret63 > 0.05)  * 15,
        int(pd.notna(ret126) and ret126 > 0.08) * 15,
    ])
    struct_pts = (15 if hh and hl else 8 if hh or hl else 0) - (15 if ll and lh else 8 if ll or lh else 0)
    vol_pen = (-10 if vol_state == "high" else -4 if vol_state == "elevated" else 0)
    dd_pen = (0 if pd.isna(dd_now) or dd_now > -0.05
              else -5 if dd_now > -0.10
              else -12 if dd_now > -0.20
              else -20 if dd_now > -0.35
              else -30)
    raw = (trend_pts + mom_pts + struct_pts + vol_pen + dd_pen +
           (rs_score - 50) * 0.2)
    score = clamp(50 + raw * 0.7)

    # Regime labels — more nuanced than before
    if score >= 78 and vol_state == "normal":
        regime = _pm("Tăng giá ổn định", "Stable uptrend")
    elif score >= 78:
        regime = _pm("Tăng giá — biến động", "Uptrend — volatile")
    elif score >= 62:
        regime = _pm("Hồi phục / tích lũy", "Recovery / accumulation")
    elif score >= 45:
        regime = _pm("Đi ngang / hỗn hợp", "Sideways / mixed")
    elif score >= 30:
        regime = _pm("Suy yếu / phân phối", "Weakening / distribution")
    else:
        regime = _pm("Rủi ro cao / giảm giá", "Risk-off / bearish")

    return {"regime": regime, "score": score, "vol_state": vol_state,
            "dd_now": dd_now, "breadth_score": rs_score,
            "trend_pts": trend_pts, "mom_pts": mom_pts, "struct_pts": struct_pts}


# ─────────────────────────── Wyckoff detection (improved) ─────────────────────
def detect_wyckoff(price_s: pd.Series, vol_s: pd.Series = None) -> Dict:
    """
    Improved Wyckoff phase detection with structural analysis:
    - Accumulation: base formation, diminishing volume, spring tests
    - Markup: trend expansion, SOS bars, LPS pullbacks
    - Distribution: topping structure, UTAD, supply bars
    - Markdown: breakdown, supply dominance
    Uses price structure + volume spread analysis (VSA).
    """
    p = price_s.dropna()
    if len(p) < 80:
        return {"phase": _pm("Chưa đủ dữ liệu", "Insufficient data"),
                "score": 50.0, "setup": _pm("Quan sát", "Observe"),
                "vsa": "", "confidence": "Low"}
    ...
    # Returns
    ret10  = p.pct_change(10).iloc[-1]  if len(p) >= 11 else np.nan
    ret20  = p.pct_change(20).iloc[-1]  if len(p) >= 21 else np.nan
    ret60  = p.pct_change(60).iloc[-1]  if len(p) >= 61 else np.nan
    ret120 = p.pct_change(120).iloc[-1] if len(p) >= 121 else np.nan

    # Range analysis
    high60  = float(p.tail(60).max())
    low60   = float(p.tail(60).min())
    high20  = float(p.tail(20).max())
    low20   = float(p.tail(20).min())
    range60 = max(high60 - low60, 1e-9)
    from60_low  = (px - low60) / range60      # 0 = at 60d low, 1 = at 60d high
    from60_high = (high60 - px) / range60     # 0 = at high, 1 = at low

    # Price structure in range (is price in upper or lower half?)
    in_upper_half = from60_low > 0.5
    in_lower_half = from60_low < 0.3
    near_high = from60_high < 0.05
    near_low  = from60_low  < 0.05

    # Trend alignment
    bull_count = sum([
        int(pd.notna(ma20)  and px > ma20),
    ...
    # Range contraction (base building indicator)
    vol_range_ratio = np.nan
    if len(p) >= 40:
        recent_range = (p.tail(20).max() - p.tail(20).min()) / max(p.tail(20).mean(), 1)
        hist_range   = (p.tail(60).max() - p.tail(60).min()) / max(p.tail(60).mean(), 1)
        vol_range_ratio = recent_range / hist_range if hist_range > 0 else np.nan

    base_forming = (pd.notna(vol_range_ratio) and vol_range_ratio < 0.55)

    # VSA: volume spread analysis
    vsa = _pm("Chưa rõ tín hiệu volume", "No clear volume-spread signal")
    vol_boost = 0.0
    spring_signal = False
    sos_signal    = False
    utad_signal   = False
    sc_signal     = False  # selling climax
    ...
    if vol_s is not None and not vol_s.empty:
        ...
        if len(v) >= 25:
            ...
            v60 = float(v.tail(60).mean()) if len(v) >= 60 else v20
            is_high_vol = v5 > v20 * 1.3
            is_low_vol  = v5 < v20 * 0.7

            # Selling Climax: high volume at multi-month lows
            if is_high_vol and near_low and pd.notna(ret20) and ret20 < -0.08:
                sc_signal = True
                vsa = _pm("Volume bùng nổ tại đáy — Selling Climax tiềm năng",
                          "Volume spike at lows — potential Selling Climax")
                vol_boost = 6.0

            # Spring test: price briefly below recent low then recovers on low vol
            elif is_low_vol and near_low and px > low20:
                spring_signal = True
                vsa = _pm("Volume thấp tại đáy range — Spring test tiềm năng",
                          "Low volume at range low — potential Spring test")
                vol_boost = 8.0

            # Sign of Strength (SOS): high volume breakout above range high
            elif is_high_vol and near_high and pd.notna(ret10) and ret10 > 0.03:
                sos_signal = True
                vsa = _pm("Volume bùng nổ breakout đỉnh range — SOS",
                          "Volume expansion at range high — Sign of Strength")
                vol_boost = 8.0

            # UTAD: high volume at new high but price reverses
            elif is_high_vol and near_high and pd.notna(ret10) and ret10 < 0:
                utad_signal = True
                vsa = _pm("Volume cao tại đỉnh nhưng giá đảo chiều — UTAD tiềm năng",
                          "High volume at top but reversing — potential UTAD")
                vol_boost = -5.0

            # Volume dry-up in base
            elif is_low_vol and in_lower_half and pd.notna(ret20) and abs(ret20) < 0.05:
                vsa = _pm("Volume co lại trong nền — dấu hiệu hấp thụ cung",
                          "Volume dry-up in base — possible supply absorption")
                vol_boost = 5.0

    # Wyckoff phase classification
    # --- Markup ---
    if bull_count >= 4 and in_upper_half and (sos_signal or (pd.notna(ret60) and ret60 > 0.15)):
        phase = "Markup"
        score = 80 + vol_boost
        if sos_signal:
            setup = _pm("SOS xác nhận — Canh LPS pullback để thêm vào", "SOS confirmed — Look for LPS pullback to add")
        else:
            setup = _pm("Breakout / LPS pullback", "Breakout / LPS pullback")
        confidence = "High" if sos_signal else "Medium"

    # --- Late Accumulation ---
    elif bull_count >= 3 and in_upper_half and base_forming and not utad_signal:
        phase = _pm("Tích lũy muộn", "Late Accumulation")
        score = 72 + vol_boost
        setup = _pm("Canh Spring hoặc LPS — chờ SOS xác nhận", "Watch for Spring or LPS — wait for SOS confirmation")
        confidence = "Medium"

    # --- Early Accumulation ---
    elif bull_count <= 2 and in_lower_half and (spring_signal or sc_signal or base_forming):
        phase = _pm("Tích lũy sớm", "Early Accumulation")
        score = 60 + vol_boost
        setup = _pm("Cần thêm thời gian để test cung. Không vội vào.", "Needs more time to test supply. Do not rush.")
        confidence = "Low"

    # --- Distribution ---
    elif bull_count <= 2 and in_upper_half and (utad_signal or (pd.notna(ret10) and ret10 < -0.03)):
        phase = _pm("Phân phối", "Distribution")
        score = 30 + vol_boost
        setup = _pm("Hạ tỷ trọng, không mua thêm — bẫy tăng giá", "Reduce exposure — potential bull trap")
        confidence = "Medium" if utad_signal else "Low"

    # --- Markdown ---
    elif bull_count <= 1 and in_lower_half and pd.notna(ret60) and ret60 < -0.12:
        phase = "Markdown"
        score = 22
        setup = _pm("Tránh bắt đáy sớm. Chờ dấu hiệu SC + Spring.", "Avoid early bottom fishing. Wait for SC + Spring signals.")
        confidence = "Medium" if sc_signal else "Low"

    # --- Range / Neutral ---
    else:
        phase = _pm("Range / trung tính", "Range / neutral")
        score = 50 + min(vol_boost, 4)
        setup = _pm("Chờ xác nhận breakout hoặc Spring test", "Wait for breakout confirmation or Spring test")
        confidence = "Low"

    return {"phase": phase, "score": clamp(score), "setup": setup,
            "vsa": vsa, "confidence": confidence,
            "spring": spring_signal, "sos": sos_signal,
            "utad": utad_signal, "sc": sc_signal,
            "base_forming": base_forming, "bull_count": bull_count}


# ─────────────────────────── Supply-demand ────────────────────────────────────
def compute_sd(price_s: pd.Series, vol_s: pd.Series = None,
               bench_s: pd.Series = None) -> Dict:
    p = price_s.dropna()
    if len(p) < 30:
        return {"score": np.nan, "label": _pm("Chưa đủ", "Insufficient"), "notes": []}
    r = p.pct_change().dropna()
    last_r = float(r.iloc[-1]) if not r.empty else np.nan
    ...
    contr = recent_abs.mean() / base_abs.mean() \
        if len(recent_abs) and len(base_abs) and base_abs.mean() != 0 else np.nan
    dry_up = clamp(100 - 110 * contr) if pd.notna(contr) else np.nan
    rs = np.nan
    if bench_s is not None and not bench_s.empty:
        df_rs = pd.concat([p.pct_change(20).rename("s"),
                           bench_s.pct_change(20).rename("b")], axis=1).dropna()
        if not df_rs.empty:
            rs = clamp(50 + 450 * float(df_rs["s"].iloc[-1] - df_rs["b"].iloc[-1]))
    pieces = {"closing": closing_str, "bp": bp, "dry_up": dry_up,
              "rs": rs if pd.notna(rs) else 50.0}
    ...
    return {"score": round(float(sc), 1) if pd.notna(sc) else np.nan,
            "label": label, "closing_str": closing_str, "bp": bp,
            "dry_up": dry_up, "rs": rs, "notes": notes}
...
def entry_engine(price_s: pd.Series, vol_s: pd.Series = None) -> Dict:
    ...
    ma50  = p.rolling(50).mean().iloc[-1] if len(p) >= 50 else np.nan
    ma200 = p.rolling(200).mean().iloc[-1] if len(p) >= 200 else np.nan
    high20 = float(p.tail(20).max()); low20 = float(p.tail(20).min())
    high10 = float(p.tail(10).max()) if len(p) >= 10 else high20
    ...
    trend_sc = float(sum([
        30 if pd.notna(ma20)  and px > ma20  else 0,
        35 if pd.notna(ma50)  and px > ma50  else 0,
        35 if pd.notna(ma200) and px > ma200 else 0,
    ]))
    mom_sc = clamp(50 + 180*(mom21 or 0) + 120*(mom63 or 0)) \
        if pd.notna(mom21) or pd.notna(mom63) else 50.0
    ...
    if entry_sc >= 75 and pd.notna(vr) and vr >= 1.2 and px >= high10 * 0.995:
        ...
        note = _pm("Vào khi giá giữ trên vùng breakout, volume không suy yếu.",
                   "Enter if price holds above breakout with volume not fading.")
        setup_tag = "Breakout"
        ...
        note = _pm("Ưu tiên mua gần MA20/support thay vì đuổi giá.",
                   "Buy near MA20/support rather than chasing extended moves.")
        setup_tag = "Pullback"
    else:
        style = _pm("Chờ xác nhận", "Wait for confirmation")
        e_low, e_high = sup_low, bko_level
        note = _pm("Chưa phải điểm vào đẹp. Chờ pullback sạch hoặc breakout rõ ràng hơn.",
                   "Not a clean entry yet. Wait for a healthier pullback or cleaner breakout.")
        setup_tag = "Watch"
    return {"entry_score": round(entry_sc, 1), "entry_low": float(e_low), "entry_high": float(e_high),
            ...
            "vol_trade_sc": round(vol_trade_sc, 1), "vol_sc": round(vol_sc, 1),
            "vol_ratio": vr}


# ─────────────────────────── Trade plan ───────────────────────────────────────
def compute_trade_plan(price_s: pd.Series, vol_s: pd.Series = None,
                       ohlcv: pd.DataFrame = None, entry_price=np.nan,
                       risk_style="swing", regime_d=None) -> Dict:
    p = price_s.dropna()
    if len(p) < 30: return {}
    px = float(p.iloc[-1])
    ma20  = p.rolling(20).mean().iloc[-1] if len(p) >= 20 else np.nan
    ma50  = p.rolling(50).mean().iloc[-1] if len(p) >= 50 else np.nan
    ma200 = p.rolling(200).mean().iloc[-1] if len(p) >= 200 else np.nan
    sup10 = float(p.tail(10).min()); sup20 = float(p.tail(20).min())
    high20 = float(p.tail(20).max())

    # Use proper True ATR if OHLCV available, otherwise approximate
    atr = atr_from_ohlcv(ohlcv) if ohlcv is not None and not ohlcv.empty else atr_approx(p)

    # ATR stop-too-tight warning
    atr_pct = atr / px if pd.notna(atr) and px > 0 else np.nan
    atr_tight_warn = (pd.notna(atr_pct) and atr_pct < 0.025)

    ee = entry_engine(p, vol_s)
    e_low  = float(ee.get("entry_low", px))
    e_high = float(ee.get("entry_high", px))
    entry  = float(entry_price) if pd.notna(entry_price) and entry_price > 0 \
        else (e_low + e_high) / 2
    wy = detect_wyckoff(p, vol_s)
    regime_d = regime_d or detect_regime(p)
    is_roff = ("risk-off" in str(regime_d.get("regime", "")).lower() or
               "giảm giá" in str(regime_d.get("regime", "")).lower())
    ...
    candidates = [x for x in [atr_stop, struct_stop, ma_stop]
    ...
    # % change from entry
    sl_pct  = (stop_loss - entry) / entry if entry > 0 else np.nan
    tp1_pct = (tp1 - entry) / entry if entry > 0 else np.nan
    tp2_pct = (tp2 - entry) / entry if entry > 0 else np.nan
    tp3_pct = (tp3 - entry) / entry if entry > 0 else np.nan
    ...
    return {"px": px, "entry_ref": entry, "entry_low": e_low, "entry_high": e_high,
            ...
            "rps": float(rps), "rr": rr, "atr": atr, "atr_pct": atr_pct,
            "atr_tight_warn": atr_tight_warn,
...
def decision_engine(ticker, ann_r, cagr_v, ann_v, shp, mdd_v, beta_v, alpha_v,
    ...
    is_roff = ("risk-off" in regime_nm.lower() or "giảm giá" in regime_nm.lower() or
               "suy yếu" in regime_nm.lower() or "weakening" in regime_nm.lower())
    ...
    sc = {
        ...
        "rel":     scale_linear(ann_r - bench_r, -0.20, 0.20)
                   if pd.notna(ann_r) and pd.notna(bench_r) else np.nan,
        "regime":  regime_sc,
    }
    wt = {"timing": 0.14, "wyckoff": 0.20, "robust": 0.10, "cagr": 0.08,
          "sharpe": 0.12, "alpha": 0.07, "vol": 0.08, "dd": 0.08,
          "cvar": 0.05, "liq": 0.10, "rel": 0.08, "regime": 0.07}
    raw = wtd_avg(sc, wt)
    base = clamp(raw or 0.0)

    # Penalties
    pen = 0.0
    if pd.notna(avg_val_20d) and avg_val_20d < 5e9 and pd.notna(ann_v) and ann_v > 0.45:
        pen += 8
    if pd.notna(rob_ret) and rob_ret < 0 and pd.notna(cagr_v) and cagr_v < 0:
        pen += 6
    if is_roff: pen += 7
    if investor_state == "holding_loss": pen += 5

    # ── DATA QUALITY PENALTY (new) ──────────────────────────────────────────
    # Missing data hurts confidence AND score
    dq_pen = 0.0
    if pd.notna(miss_pct) and miss_pct > 0.20:   dq_pen += 10
    elif pd.notna(miss_pct) and miss_pct > 0.10: dq_pen += 5
    if pd.notna(ffill_pct) and ffill_pct > 0.15:  dq_pen += 8
    elif pd.notna(ffill_pct) and ffill_pct > 0.08: dq_pen += 4
    if data_pts < 60:  dq_pen += 12
    elif data_pts < 120: dq_pen += 6
    pen += dq_pen
    ...
    if pd.notna(avg_val_20d) and avg_val_20d < 1e9:
        final = min(final, 28.0)
        notes.append(_pm("Thanh khoản quá thấp.", "Liquidity too low."))
    elif pd.notna(avg_val_20d) and avg_val_20d < 5e9:
        final = min(final, 54.0)
        notes.append(_pm("Thanh khoản yếu.", "Weak liquidity."))
    if is_roff:
        cap = 58.0 if investor_state == "holding_gain" else 52.0
        final = min(final, cap)
        notes.append(_pm("Bối cảnh regime yếu/risk-off.", "Weak/risk-off regime."))
    ...
    if timing_overall == wait_lbl:
        ...
        notes.append(_pm("Timing yếu.", "Timing is weak."))
    ...
    if miss_pct and miss_pct > 0.10:
        final = min(final, 44.0)
        notes.append(_pm("Dữ liệu thiếu nhiều.", "High missing data."))
    if investor_state == "holding_loss":
        final = min(final, 48.0)

    final = round(clamp(final), 1)

    # ── CONFIDENCE SCORE (data-quality weighted) ────────────────────────────
    avail = sum(pd.notna(v) for v in sc.values()); total = len(sc)
    comp = 100 * avail / total if total else 0
    hist_sc = (100 if data_pts >= 252 else 80 if data_pts >= 180
               else 60 if data_pts >= 120 else 40 if data_pts >= 60 else 20)
    ...
    # Data quality component (new — explicit penalty to confidence)
    dq_sc = 100.0
    if pd.notna(miss_pct):  dq_sc -= miss_pct * 200   # -20 per 10% missing
    if pd.notna(ffill_pct): dq_sc -= ffill_pct * 150
    if data_pts < 60:       dq_sc -= 30
    elif data_pts < 120:    dq_sc -= 15
    dq_sc = max(dq_sc, 0)

    conf_raw = clamp(0.35 * comp + 0.25 * hist_sc + 0.15 * liq_cs + 0.25 * dq_sc)
    conf_lbl = (_pm("Cao", "High") if conf_raw >= 75
                else _pm("Trung bình", "Medium") if conf_raw >= 50
                else _pm("Thấp", "Low"))

    # Reasons / risks
    reasons, risks = [], []
    if pd.notna(wy_score):
        if wy_score >= 75: reasons.append(_pm(f"Wyckoff setup thuận lợi ({wy_phase})", f"Favorable Wyckoff setup ({wy_phase})"))
        elif wy_score < 45: risks.append(_pm(f"Wyckoff yếu ({wy_phase})", f"Weak Wyckoff ({wy_phase})"))
    if pd.notna(rob_ret):
    ...
    if dq_pen > 0: risks.append(_pm(f"Chất lượng dữ liệu thấp (penalty -{dq_pen:.0f}đ)",
                                     f"Low data quality (penalty -{dq_pen:.0f}pt)"))
    ...
    if pd.notna(avg_val_20d) and avg_val_20d < 1e9:
        ...
        size = 0.03; size_lbl = _pm("Tối đa 3%", "Max 3% / weak liq")
    ...
    if investor_state == "holding_loss":
        size = 0.0; size_lbl = _pm("Phòng thủ / không thêm", "Defend / do not add")
...
# ─────────────────────────── Master verdict ───────────────────────────────────
def master_verdict(action_d: Dict, timing_d: Dict, avg_val_20d: float,
                   investor_state="new") -> Dict:
    ...
    if investor_state == "holding_gain":
        if score >= 80 and timing_ov == buy_sig and not is_roff:
            return {"label": BUY, "score": score, "tone": "good",
                    "reason": _pm("Vị thế lãi và setup tiếp tục xác nhận — cộng lệnh có kỷ luật.",
                                  "Winning position and setup still confirmed — disciplined add-on justified.")}
        if score >= 58:
            return {"label": WTCH, "score": score, "tone": "warn",
                    "reason": _pm("Ưu tiên bảo vệ lợi nhuận bằng trailing stop.",
                                  "Protect gains with a trailing stop.")}
        return {"label": RDCE, "score": score, "tone": "warn",
                "reason": _pm("Lợi thế suy yếu — xem xét chốt bớt.",
                               "Edge weakening — consider trimming.")}
    if investor_state == "holding_loss":
        if score >= 82 and timing_ov == buy_sig and not is_roff:
            return {"label": STRT, "score": score, "tone": "warn",
                    "reason": _pm("Chỉ cộng rất nhỏ khi có reclaim mạnh và luận điểm rõ.",
                                  "Only a very small add-on if there is a strong reclaim.")}
        if score >= 48:
            return {"label": WTCH, "score": score, "tone": "warn",
                    "reason": _pm("Chờ tín hiệu phục hồi rõ hơn. Tôn trọng stop.",
                                  "Wait for a clearer recovery. Respect the stop.")}
        return {"label": AVOD, "score": score, "tone": "bad",
                "reason": _pm("Cấu trúc không hỗ trợ — ưu tiên thoát / giảm rủi ro.",
                               "Structure does not support — prioritize exit or de-risk.")}
    # New position
    if score >= 82 and timing_ov == buy_sig and not is_roff and \
       (pd.isna(avg_val_20d) or avg_val_20d >= 5e9):
        return {"label": BUY, "score": score, "tone": "good",
                "reason": _pm("Điểm số, timing, thanh khoản và regime cùng xác nhận.",
                               "Score, timing, liquidity, and regime all confirm.")}
    if score >= 66 and timing_ov in [buy_sig, wtch_sig] and not is_roff:
        return {"label": STRT, "score": score, "tone": "good",
                "reason": _pm("Setup đủ tốt cho vị thế nhỏ. Chờ xác nhận trước khi tăng.",
                               "Good enough for a starter. Wait for confirmation before adding.")}
    if score >= 46:
        return {"label": WTCH, "score": score, "tone": "warn",
                "reason": _pm("Cần thêm xác nhận từ giá, volume hoặc regime.",
                               "More confirmation needed from price, volume, or regime.")}
    return {"label": AVOD, "score": score, "tone": "bad",
            "reason": _pm("Rủi ro lớn hơn phần thưởng tại thời điểm này.",
                           "Risk outweighs expected reward right now.")}
...
def next_step_text(label: str) -> str:
    ...
    RDCE = _pm("Chốt / giảm", "Reduce / TP")
    AVOD = _pm("Tránh / thoát", "Avoid / Exit")
    if label == BUY:  return _pm("Mở Position Manager, chốt entry + stop + plan giải ngân.",
                                  "Open Position Manager, lock entry + stop + execution plan.")
    if label == STRT: return _pm("Vào nhỏ, chờ breakout hoặc pullback test thành công.",
                                  "Enter small, wait for breakout or successful pullback test.")
    if label == RDCE: return _pm("Ưu tiên hạ rủi ro. Không thêm lệnh.",
                                  "Prioritize de-risking. Do not add.")
    if label == AVOD: return _pm("Bảo toàn vốn. Chờ cơ hội sạch hơn.",
                                  "Preserve capital. Wait for a cleaner opportunity.")
    return _pm("Đặt cảnh báo giá, không vội vào lệnh.",
               "Set price alerts, do not force an entry.")
...
def exec_checklist(master_d: Dict, trade_d: Dict) -> List[str]:
    ...
    RDCE = _pm("Chốt / giảm", "Reduce / TP")
    if lbl == BUY:
        steps.append(_pm("Xác nhận giá còn trong vùng entry và volume không hụt.",
                          "Confirm price is still in the entry zone and volume has not collapsed."))
        steps.append(_pm("Vào lệnh 1-2 nhịp thay vì all-in một lần.",
                          "Scale in 1-2 clips instead of all-in."))
    elif lbl == STRT:
        steps.append(_pm("Mở vị thế nhỏ để test luận điểm.",
                          "Open a small position to test the thesis."))
        steps.append(_pm("Chỉ tăng thêm sau khi có breakout hoặc test thành công.",
                          "Only add after a breakout or successful test."))
    elif lbl == RDCE:
        steps.append(_pm("Ưu tiên bảo vệ vốn, không thêm mới.",
                          "Prioritize capital protection, no new exposure."))
    else:
        steps.append(_pm("Đặt cảnh báo giá/volume thay vì vào sớm.",
                          "Set price/volume alerts instead of entering early."))
    if trade_d:
        sl_str = f"{fmt_px(trade_d.get('stop_loss'))} ({fmt_pct(trade_d.get('sl_pct'))})"
        tp2_str = f"{fmt_px(trade_d.get('tp2'))} ({fmt_pct(trade_d.get('tp2_pct'))})"
        steps.append(f"Stop: {sl_str} | TP2: {tp2_str} | R/R: {fmt_num(trade_d.get('rr'))}R")
        if trade_d.get("atr_tight_warn"):
            steps.append(_pm("⚠️ ATR/stop rất gần giá — rủi ro bị hit sớm trong phiên bình thường.",
                              "⚠️ ATR/stop is very tight — risk of premature stop-out on normal volatility."))
    return steps[:4]


# ─────────────────────────── Smart alerts ─────────────────────────────────────
def smart_alerts(ticker, price_s, timing_d, regime_d, action_d,
                 avg_val_20d, ffill_pct) -> List[str]:
    alerts = []
    p = price_s.dropna()
    if len(p) >= 3:
        drop = p.iloc[-1] / p.iloc[-3] - 1
        if drop <= -0.07:
            alerts.append(_pm(f"Giảm mạnh 3 phiên: {drop:.1%}",
                               f"Sharp 3-session drop: {drop:.1%}"))
    ...
    if regime_d:
        reg = str(regime_d.get("regime", "")).lower()
        if "risk-off" in reg or "giảm giá" in reg or "suy yếu" in reg or "weakening" in reg:
            alerts.append(_pm("Regime yếu — giảm rủi ro", "Weak regime — reduce risk"))
...
# ─────────────────────────── Full analysis pack ───────────────────────────────
def build_analysis_pack(ticker, prices, volumes, metrics_df, bench_r, alpha_conf,
                        ohlcv_cache: Dict = None) -> Dict:
    p  = prices[ticker]
    vs = get_vol_series(volumes, ticker)
    bench_s = (prices[VNINDEX_SYMBOL] if VNINDEX_SYMBOL in prices.columns
               else (prices.iloc[:, 0] if not prices.empty else pd.Series(dtype=float)))
    investor_state = infer_investor_state_for_ticker(ticker, prices)
    timing_d = compute_timing(p, vs)
    regime_d = detect_regime(p, bench_s)
    ...
    # Pass OHLCV to trade plan if available for true ATR
    ohlcv = ohlcv_cache.get(ticker) if ohlcv_cache else None
    trade_d  = compute_trade_plan(p, vs, ohlcv=ohlcv, regime_d=regime_d)
    mv_d     = master_verdict(action_d, timing_d, avg_val, investor_state)
    rsi_s    = calc_rsi(p.dropna())
    rsi_v    = float(rsi_s.dropna().iloc[-1]) if not rsi_s.dropna().empty else np.nan
...
def price_volume_chart(ticker, start, end, source, height=520) -> go.Figure:
    ...
    hist = hist.copy()
    for ma, col in [(20, "#f0a500"), (50, "#0060bb"), (200, "#6f42c1")]:
        hist[f"MA{ma}"] = hist["close"].rolling(ma).mean()
    vol_c = np.where(hist["close"] >= hist["open"],
                     "rgba(29,158,117,.65)", "rgba(192,48,48,.65)")
    ...
    fig.add_trace(go.Bar(x=hist["date"], y=hist["volume"], name="Vol",
                         marker_color=vol_c), row=2, col=1)
...
def rsi_chart(price_s: pd.Series, ticker: str, height=160) -> go.Figure:
    ...
    fig.update_layout(height=height, title=f"RSI (14) — {ticker}",
...
def wl_add(ticker, snap):
    st.session_state.watchlist[ticker] = snap; _persist_wl()
...
def _persist_wl():
    try:
        st.session_state["_wl_json"] = json.dumps(
            {k: {ky: (None if isinstance(vy, float) and np.isnan(vy) else vy)
...
def pb_save(ticker, entry_px, shares, style="swing"):
    ...
    try:
        st.session_state["_pb_json"] = json.dumps(book, ensure_ascii=False)
    except Exception:
        pass
...
def pb_delete(ticker):
    ...
    try:
        st.session_state["_pb_json"] = json.dumps(st.session_state.position_book, ensure_ascii=False)
    except Exception:
        pass


def manage_position(price_s, entry_px, shares, trade_d) -> Dict:
    if not trade_d or pd.isna(entry_px) or entry_px <= 0 or \
       pd.isna(shares) or shares <= 0:
        return {}
    p = price_s.dropna()
    if p.empty: return {}
    px = float(p.iloc[-1])
    ...
    if px <= sl:
        ...
        note = _pm("Giá chạm vùng vô hiệu hóa setup. Không nới stop.",
                   "Price hit the setup invalidation level. Do not widen the stop.")
    elif px >= tp3:
        action = _pm("Chốt mạnh / giữ runner", "Take major profit / keep runner")
        note = _pm("Giá đi xa. Chốt phần lớn, giữ phần nhỏ bằng trailing stop.",
                   "Price has moved far. Take most profit, trail the remainder.")
    elif px >= tp2:
        action = _pm("Chốt từng phần tại TP2", "Partial take profit at TP2")
        note = _pm("Đã đạt TP2. Chốt 30-50%, dời stop về trên giá vốn.",
                   "TP2 reached. Take 30-50%, move stop above breakeven.")
    elif px >= tp1:
        action = _pm("Giữ, cân nhắc bảo vệ lãi", "Hold, consider protecting gains")
        note = _pm("Đạt TP1. Có thể dời stop về hoà vốn.",
                   "TP1 hit. Consider moving stop to breakeven.")
    elif px <= trail and pnl_pct > 0:
        action = _pm("Chốt dần / kéo trailing stop", "Trim and trail")
        note = _pm("Đang lãi nhưng giá về trailing stop. Xem xét chốt bớt.",
                   "In profit but pulling back to trailing stop. Consider trimming.")
    elif pnl_pct <= -0.08:
        action = _pm("Giảm rủi ro / xem xét thoát", "De-risk / consider exiting")
        note = _pm("Vị thế lỗ đáng kể. Tôn trọng stop, không bình quân giá.",
                   "Significantly underwater. Respect the stop, no averaging down.")
    else:
        action = _pm("Tiếp tục nắm giữ", "Continue holding")
        note = _pm("Cấu trúc chưa gãy. Theo dõi với stop hiện tại.",
                   "Structure intact. Monitor with current stop.")
...
def portfolio_heat(position_book, prices_df) -> Dict:
    if not position_book: return {"positions": 0}
    rows = []
    for tk, pos in position_book.items():
        ...
        if sh <= 0 or ep <= 0: continue
        mv = px * sh; cost = ep * sh
        pnl_pct = px / ep - 1; pnl_val = mv - cost
        rows.append({"Ticker": tk, "Market Value": mv, "Cost Value": cost,
                     "PnL": pnl_pct, "PnL Value": pnl_val, "Entry": ep})
    if not rows: return {"positions": 0}
    ...
    tickers = df["Ticker"].tolist(); avg_corr = np.nan
    if len(tickers) >= 2:
    ...
    return {"positions": len(rows), "market_value": total_mv, "cost_value": total_cost,
            "portfolio_pnl_val": total_pnl_val, "portfolio_pnl_pct": total_pnl_pct,
            "avg_corr": avg_corr, "rows": df.to_dict("records")}


def risk_based_sizing(capital, risk_pct, entry, stop) -> Dict:
    if capital <= 0 or pd.isna(entry) or pd.isna(stop) or entry <= stop:
        return {"shares": np.nan, "capital_req": np.nan,
                "weight": np.nan, "risk_amt": np.nan}
    ...
    return {"shares": shares, "capital_req": cap_req,
            "weight": weight, "risk_amt": risk_amt}
...
def score_bar_html(score, color="#0060bb") -> str:
    ...
    return (f"<div class='bar-wrap'>"
            f"<div class='bar-fill' style='width:{w:.0f}%;background:{c}'></div>"
            f"</div>")
...
def score_breakdown_html(sc_d: Dict) -> str:
    ...
    for key, lbl in display.items():
        ...
        html += (f"<div class='bm-label'>{lbl}</div>"
                 f"<div class='bm-row'><div class='bm-track'>"
                 f"<div class='bm-fill' style='width:{w:.0f}%;background:{c}'></div></div>"
                 f"<div class='bm-val' style='color:{c}'>{w:.0f}</div></div>")
    return html


def tbl_row(label, value) -> str:
    return (f"<div class='tbl-row'><span class='tbl-label'>{label}</span>"
            f"<span class='tbl-val'>{escape(str(value))}</span></div>")


def tbl_row_html(label, value_html: str) -> str:
    """tbl_row but value is already HTML (not escaped)."""
    return (f"<div class='tbl-row'><span class='tbl-label'>{label}</span>"
            f"<span class='tbl-val'>{value_html}</span></div>")
...
with st.sidebar:
    ...
    if VNSTOCK_AUTH.get("ok"):
        st.caption(_pm("Vnstock Community API: đã bật.", "Vnstock Community API: enabled."))
    else:
        st.caption(_pm(f"Vnstock API auth chưa bật: {VNSTOCK_AUTH.get('message', 'unknown')}",
                       f"Vnstock API auth not active: {VNSTOCK_AUTH.get('message', 'unknown')}"))
    ...
    with st.expander("⚙️ " + ("Nâng cao" if lang() == "vi" else "Advanced"), expanded=False):
        rf_annual   = st.number_input(t("rf"), min_value=0.0, max_value=1.0,
                                      value=DEFAULT_RF, step=0.005)
        ...
        mc_sims     = st.slider(t("mc_sims"), min_value=1000, max_value=10000,
                                step=1000, value=3000)
        show_valuation = st.checkbox(
            _pm("Hiển thị định giá TCBS (cần mạng)", "Show TCBS valuation (needs network)"),
            value=True)
        show_foreign = st.checkbox(
            _pm("Hiển thị dòng tiền ngoại", "Show foreign flow"),
            value=True)

    st.markdown("---")
    port_cap = st.number_input(t("capital"), min_value=0.0,
                               value=float(st.session_state.portfolio_capital),
...
if not st.session_state.ran:
    ...
    <div style='max-width:680px;margin:48px auto 0;text-align:center'>
      ...
      <p style='opacity:.65;font-size:.92rem'>Nhập mã trọng tâm ở thanh bên trái để dùng Radar/Workspace,
      hoặc mở System → Screener để quét cả thị trường.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    for col, icon, lbl, desc in [
        ...
        with col:
            st.markdown(
                ...
                f"<div style='font-size:1.8rem'>{icon}</div><b>{lbl}</b>"
                f"<p style='font-size:.75rem;opacity:.65;margin:4px 0 0'>{desc}</p></div>",
                unsafe_allow_html=True)
    st.stop()
...
if len(tickers) > MAX_FOCUS_TICKERS:
    st.warning(_pm(f"Chỉ dùng tối đa {MAX_FOCUS_TICKERS} mã. App sẽ lấy {MAX_FOCUS_TICKERS} mã đầu tiên.",
                   f"Only the first {MAX_FOCUS_TICKERS} focus tickers will be used."))
...
simple_rets = prices[asset_cols].pct_change().dropna(how="all")
log_rets    = np.log(prices[asset_cols] / prices[asset_cols].shift(1)).dropna(how="all")
bench_s = (prices[benchmark].pct_change().rename(benchmark)
           if benchmark in prices.columns else pd.Series(dtype=float))

raw_na  = prices_raw[asset_cols].isna().sum()
ffill_a = (prices[asset_cols].notna().sum() - prices_raw[asset_cols].notna().sum()).clip(lower=0)
overall_last = (pd.to_datetime([d for d in last_dates.values() if d is not None]).max()
                if any(d for d in last_dates.values()) else None)

# Data warnings
dwarns = []
for col in asset_cols:
    if row_counts.get(col, 0) < 60:
        dwarns.append(f"{col}: < 60 phiên")
    if last_dates.get(col) and overall_last:
        gap = (overall_last - last_dates[col]).days
        if gap >= 3:
            dwarns.append(f"{col}: lag {gap} ngày ({last_dates[col].strftime('%d/%m/%Y')})")

bench_ret   = ann_return(bench_s) if not bench_s.empty else np.nan
bench_vol_v = (ann_vol(np.log(prices[benchmark] / prices[benchmark].shift(1)).dropna())
               if benchmark in prices.columns else np.nan)
...
exp_rets = np.array([robust_ret(simple_rets[c]) or (simple_rets[c].mean() * TDAYS)
                     for c in asset_cols])
cov_matrix = np.nan_to_num(log_rets[asset_cols].cov(min_periods=30).values * TDAYS)

# ─── Weights ──────────────────────────────────────────────────────────────────
ensure_weights(asset_cols)
if st.session_state.applied_weights is None or \
   len(st.session_state.applied_weights) != len(asset_cols):
...
# ─── OHLCV cache for true ATR ─────────────────────────────────────────────────
@st.cache_data(show_spinner=False)
def _load_ohlcv_cache(tickers_key: str, start: date, end: date, source: str) -> Dict:
    cache = {}
    for tk in tickers_key.split(","):
        ohlcv, _ = _fetch_ohlcv(tk, start, end, source)
        if not ohlcv.empty:
            cache[tk] = ohlcv
    return cache

ohlcv_cache = _load_ohlcv_cache(",".join(asset_cols), start_date, end_date, data_source)
...
# ─── Header snapshot ──────────────────────────────────────────────────────────
# Unadjusted price warning — always shown
st.markdown(
    f"<div class='warn-box'>⚠️ <b>{t('unadj_warn')}</b></div>",
    unsafe_allow_html=True)
...
best_st = (metrics_df["ann_ret"].idxmax()
           if "ann_ret" in metrics_df.columns and not metrics_df["ann_ret"].isna().all()
           else "N/A")
...
with tab_radar:
    ...
    # Score disclaimer
    st.markdown(
        f"<div class='disclaimer-box'>ℹ️ <b>{t('score_note')}</b> — "
        + _pm("Điểm cao không đảm bảo lợi nhuận. Luôn kiểm tra định giá, tin tức, và quản trị rủi ro.",
              "High score does not guarantee profit. Always verify valuation, news, and risk management.")
        + "</div>", unsafe_allow_html=True)

    with st.expander(_pm("🧩 Cập nhật bối cảnh NĐT theo từng mã",
                          "🧩 Update investor context by ticker"), expanded=False):
    ...
    for col in asset_cols:
        ...
        radar_rows.append({
            ...
            t("regime"):    pack.get("regime", {}).get("regime", "N/A"),
            "Wyckoff":      pack.get("wyckoff", {}).get("phase", "N/A"),
    ...
    st.dataframe(radar_df, width='stretch', height=min(55 + 36*len(asset_cols), 420))
    ...
    if buy_tickers:
        st.markdown(
            f"<div class='tip'>✅ <b>{_pm('Setup tốt nhất hiện tại', 'Best setups right now')}:</b> "
            f"{', '.join(buy_tickers[:3])} — "
            f"{_pm('Mở Workspace để ra quyết định chi tiết.', 'Open Workspace for detailed analysis.')}</div>",
            unsafe_allow_html=True)
    ...
    n_buy_timing = sum(1 for col in asset_cols
                       if analysis_cache[col].get("timing", {}).get("overall", "") ==
                       _pm("🟢 Vào hàng", "🟢 Buy"))
    avg_regime_sc = float(np.nanmean([analysis_cache[col].get("regime", {}).get("score", np.nan)
                                      for col in asset_cols]))
    ...
    with b3: st.metric(_pm("Regime TB", "Avg regime"),
                       f"{avg_regime_sc:.0f}/100" if pd.notna(avg_regime_sc) else "N/A")

    # ── Cumulative returns ──────────────────────────────────────────────────────
    st.markdown("---")
    st.markdown(f"#### {_pm('📈 Lợi nhuận tích lũy (giá chưa điều chỉnh cổ tức)', '📈 Cumulative returns (unadjusted)')}")
...
with tab_ws:
    ...
    st.markdown(
        f"<div class='disclaimer-box'>ℹ️ <b>{t('score_note')}</b></div>",
        unsafe_allow_html=True)
    ...
    va, vb_col = st.columns([1.45, 0.85])
    with va:
        ...
        dq_pen = wa.get("dq_penalty", 0)
        dq_note = (f" <small style='color:#c07800'>(DQ penalty: -{dq_pen:.0f}pt)</small>"
                   if dq_pen > 0 else "")
    ...
    # ── Section B: Chart + Trade Plan + Valuation/Foreign ─────────────────────
    ...
    with bd:
        # Trade plan with % change
        st.markdown(f"#### {t('trade_plan')}")
        if wtr:
            entry_ref = wtr.get("entry_ref", last_px)

            def level_str(px_val, pct_val):
                if pd.isna(px_val): return "N/A"
                pct_html = fmt_pct_chg(px_val, entry_ref)
                return f"{fmt_px(px_val)} {pct_html}"

            plan_html = (
                tbl_row(t("entry_zone"),   wtr.get("entry_zone_text", "N/A"))
                + tbl_row_html(t("stop"),  level_str(wtr.get("stop_loss"),  wtr.get("sl_pct")))
                + tbl_row_html("TP1",      level_str(wtr.get("tp1"),        wtr.get("tp1_pct")))
                + tbl_row_html("TP2",      level_str(wtr.get("tp2"),        wtr.get("tp2_pct")))
                + tbl_row_html("TP3",      level_str(wtr.get("tp3"),        wtr.get("tp3_pct")))
                + tbl_row(t("trail_stop"), fmt_px(wtr.get("trailing_stop")))
                + tbl_row(t("rr"),         f"{fmt_num(wtr.get('rr'))}R")
                + tbl_row(t("wyckoff"),    f"{wtr.get('wyckoff_phase','')} ({fmt_num(wtr.get('wyckoff_score'))}%)")
                + tbl_row(_pm("Setup", "Setup"), wtr.get("setup_tag", ""))
            )
            atr_val = wtr.get("atr", np.nan)
            atr_pct = wtr.get("atr_pct", np.nan)
            if pd.notna(atr_val):
                plan_html += tbl_row("ATR (True)",
                                     f"{fmt_px(atr_val)} ({fmt_pct(atr_pct)})")
            st.markdown(f"<div class='card'>{plan_html}</div>", unsafe_allow_html=True)
            if wtr.get("atr_tight_warn"):
                st.markdown(
                    f"<div class='warn-box'>⚠️ {_pm('Stop rất gần — rủi ro bị hit sớm trong biến động bình thường', 'Stop is very tight — risk of premature stop-out')}</div>",
                    unsafe_allow_html=True)
            st.markdown(f"<div class='tip'>{escape(wtr.get('entry_note', ''))}</div>",
                        unsafe_allow_html=True)
        else:
        ...
        for note in ws_d.get("notes", [])[:2]:
            st.markdown(f"<div class='tip' style='margin:3px 0'>{escape(note)}</div>",
                        unsafe_allow_html=True)
        ...
        est_shares = (int(np.floor(cap_plan / last_px))
                      if pd.notna(last_px) and last_px > 0 else 0)
        sz_html = (tbl_row(t("size_label"),   pos.get("label", "N/A"))
                   + tbl_row(t("capital_plan"), f"{cap_plan:,.0f} VND")
                   + tbl_row(t("shares_est"),   f"{est_shares:,} cp"))
        st.markdown(f"<div class='card'>{sz_html}</div>", unsafe_allow_html=True)

        # ── Valuation (TCBS) ──────────────────────────────────────────────────
        if show_valuation:
            st.markdown(f"#### {t('valuation')}")
            val_data = fetch_valuation_tcbs(ws_sel)
            st.markdown(valuation_html(val_data, lang()), unsafe_allow_html=True)

        # ── Foreign flow ──────────────────────────────────────────────────────
        if show_foreign:
            st.markdown(f"#### {t('foreign')}")
            fd = fetch_foreign_data(ws_sel)
            st.markdown(foreign_html(fd, lang()), unsafe_allow_html=True)
    ...
    with rc:
        ...
        for r_ in wa.get("reasons", []) or [_pm("Chưa rõ", "N/A")]:
            st.markdown(
                f"<div class='tip' style='margin:3px 0;border-left-color:rgba(0,180,100,.5)'>✅ {escape(r_)}</div>",
                unsafe_allow_html=True)
    with rd:
        st.markdown(f"**{t('risks')}**")
        for r_ in wa.get("risks", []) or [_pm("Chưa rõ", "N/A")]:
            st.markdown(
                f"<div class='tip' style='margin:3px 0;border-left-color:rgba(220,50,50,.5)'>⚠️ {escape(r_)}</div>",
                unsafe_allow_html=True)
    with re_:
        st.markdown(f"**{t('exec_check')}**")
        for i, s in enumerate(wp.get("exec_steps", []), 1):
            st.markdown(f"<div style='font-size:.80rem;padding:3px 0'>{i}. {escape(s)}</div>",
                        unsafe_allow_html=True)
        if wp.get("alerts"):
            st.markdown(f"**⚡ Alerts**")
            for a_ in wp["alerts"]:
                st.markdown(f"<span class='pill p-red'>⚠ {escape(a_)}</span>",
                            unsafe_allow_html=True)
    ...
    with m1: st.metric(t("return_yr"),  fmt_pct(row.get("ann_ret")),
                       fmt_pct(row.get("ann_ret", np.nan) - bench_ret)
                       if pd.notna(row.get("ann_ret")) and pd.notna(bench_ret) else None)
    ...
    with m10: st.metric("CVaR",        fmt_pct(row.get(f"cvar_{int(alpha_conf*100)}")))
    ...
    # Data quality notice
    miss_p = row.get("miss_pct", 0)
    ffill_p = row.get("ffill_pct", 0)
    dq_issues = []
    if pd.notna(miss_p) and miss_p > 0.05:
        dq_issues.append(_pm(f"Thiếu dữ liệu: {miss_p:.1%}", f"Missing data: {miss_p:.1%}"))
    if pd.notna(ffill_p) and ffill_p > 0.05:
        dq_issues.append(_pm(f"Forward-fill: {ffill_p:.1%}", f"Forward-fill: {ffill_p:.1%}"))
    if dq_issues:
        st.markdown(
            f"<div class='warn-box'>⚠️ <b>{_pm('Chất lượng dữ liệu', 'Data quality')}:</b> "
            + " | ".join(dq_issues)
            + f" — {_pm('Các chỉ số có thể bị sai lệch.', 'Metrics may be distorted.')}</div>",
            unsafe_allow_html=True)
    ...
    with pm2:
        pm_style = st.selectbox(_pm("Phong cách", "Style"),
                                ["tight", "swing", "position"], index=1,
                                key=f"pm_style_{ws_sel}")
    with pm3:
        pm_entry = st.number_input(_pm("Giá vốn / giá dự kiến", "Entry price"),
                                   min_value=0.0, value=0.0, step=100.0,
                                   key=f"pm_entry_{ws_sel}")
    pm_shares = 0.0
    ...
    if pm_trade:
        ...
        with pm_a:
            entry_r = pm_trade.get("entry_ref", last_px)
            plan_html_pm = (
                tbl_row(t("current_price"), fmt_px(pm_trade.get("px")))
                + tbl_row_html(t("stop"),   level_str(pm_trade.get("stop_loss"), pm_trade.get("sl_pct")) if wtr else fmt_px(pm_trade.get("stop_loss")))
                + tbl_row(t("trail_stop"),  fmt_px(pm_trade.get("trailing_stop")))
                + tbl_row_html("TP1",       f"{fmt_px(pm_trade.get('tp1'))} {fmt_pct_chg(pm_trade.get('tp1'), entry_r)}")
                + tbl_row_html("TP2",       f"{fmt_px(pm_trade.get('tp2'))} {fmt_pct_chg(pm_trade.get('tp2'), entry_r)}")
                + tbl_row_html("TP3",       f"{fmt_px(pm_trade.get('tp3'))} {fmt_pct_chg(pm_trade.get('tp3'), entry_r)}")
                + tbl_row(t("rr"),          f"{fmt_num(pm_trade.get('rr'))}R")
            )
            st.markdown(f"<div class='card'>{plan_html_pm}</div>", unsafe_allow_html=True)
        with pm_b:
            sz_html2 = (
                tbl_row(_pm("Vốn rủi ro / lệnh", "Risk budget"), f"{fmt_px(sizing_pm.get('risk_amt'))} VND")
                + tbl_row(_pm("Số cổ phiếu", "Shares"), f"{sizing_pm.get('shares','N/A'):,}" if pd.notna(sizing_pm.get('shares')) else "N/A")
                + tbl_row(_pm("Vốn cần", "Capital"), f"{fmt_px(sizing_pm.get('capital_req'))} VND")
                + tbl_row(_pm("Tỷ trọng", "Weight"), fmt_pct(sizing_pm.get('weight')))
            )
            st.markdown(f"<div class='card'>{sz_html2}</div>", unsafe_allow_html=True)
        ...
        with ps_a:
            if st.button(t("save_pos"), key=f"save_pos_{ws_sel}",
                         disabled=not(_pm("Đang giữ", "Holding") in pm_mode
                                      and pm_shares > 0 and pm_entry > 0)):
    ...
    if pb:
        ...
        for tk, pos in pb.items():
            if tk not in asset_cols: continue
            tk_px = (float(prices[tk].dropna().iloc[-1])
                     if tk in prices.columns and not prices[tk].dropna().empty else np.nan)
            ep = float(pos.get("entry_price", 0))
            sh = float(pos.get("shares", 0))
    ...
    # ── Timing / Wyckoff detail ────────────────────────────────────────────────
    st.markdown("---")
    st.markdown(f"#### {t('timing')}")
    if wt:
        st.markdown(
            f"<div class='sig-card {wt.get('cls','sig-watch')}'>"
            f"<b>{escape(ws_sel)}</b> — {escape(wt.get('overall','N/A'))} "
            f"({wt.get('score',0)}/{wt.get('max_score',0)})<br>"
            + "".join([
                f"<span class='pill {'p-green' if any(c in s for c in ['✅','🏆','📈']) else 'p-red'}'>{escape(s)}</span>"
                for s in wt.get("signals", [])
            ])
            + "</div>", unsafe_allow_html=True)

    wy_conf = wy.get("confidence", "")
    wy_conf_cls = "p-green" if wy_conf == "High" else ("p-yellow" if wy_conf == "Medium" else "p-red")
    st.markdown(
        f"**{t('wyckoff')}:** {escape(wy.get('phase','N/A'))} "
        f"<span class='pill {wy_conf_cls}'>{wy_conf}</span> "
        f"(score: {fmt_num(wy.get('score'))}) — {escape(wy.get('setup',''))}",
        unsafe_allow_html=True)
    if wy.get("vsa"):
        st.markdown(f"<div class='tip'>{escape(wy.get('vsa',''))}</div>", unsafe_allow_html=True)
    # Wyckoff signal pills
    sig_pills = ""
    if wy.get("spring"): sig_pills += "<span class='pill p-green'>🌱 Spring</span>"
    if wy.get("sos"):    sig_pills += "<span class='pill p-green'>💪 SOS</span>"
    if wy.get("utad"):   sig_pills += "<span class='pill p-red'>⚠️ UTAD</span>"
    if wy.get("sc"):     sig_pills += "<span class='pill p-yellow'>📉 Selling Climax</span>"
    if wy.get("base_forming"): sig_pills += "<span class='pill p-blue'>🏗️ Base forming</span>"
    if sig_pills:
        st.markdown(sig_pills, unsafe_allow_html=True)
...
with tab_pf:
    ...
    presets = [("eq", t("preset_eq")), ("min", t("preset_min")),
               ("tan", t("preset_tan")), ("rp", t("preset_rp"))]
    ...
    # ── Portfolio metrics ──────────────────────────────────────────────────────
    al_sr = simple_rets[asset_cols].dropna()
    al_lr = log_rets[asset_cols].dropna()
    ...
    with pm1: st.metric(t("return_yr"), fmt_pct(pf["ann_return"]),
                        fmt_pct(pf["ann_return"]-bench_ret)
                        if pd.notna(pf["ann_return"]) and pd.notna(bench_ret) else None)
    ...
    if pd.notna(pf["var95"]):
        st.markdown(
            f"<div class='tip'>📌 <b>{_pm('Rủi ro ngày xấu','Bad-day risk')}:</b> "
            f"{_pm('Trong','In')} {int(alpha_conf*100)}% "
            f"{_pm('ngày tệ nhất, danh mục có thể giảm hơn','worst days, portfolio may lose more than')} "
            f"<b>{abs(pf['var95']):.1%}</b>. "
            f"{_pm('Trung bình các ngày đó:','Average on those days:')} <b>{abs(pf['cvar95']):.1%}</b>.</div>",
            unsafe_allow_html=True)

    pf_c1, pf_c2 = st.columns(2)
    with pf_c1:
        st.markdown(f"#### {_pm('📈 Danh mục vs Benchmark (giá chưa điều chỉnh)', '📈 Portfolio vs Benchmark (unadjusted)')}")
        fig_pf = go.Figure()
        if not pf["wealth"].empty:
            fig_pf.add_trace(go.Scatter(x=pf["wealth"].index, y=pf["wealth"]-1,
                mode="lines", name=_pm("Danh mục", "Portfolio")))
        if benchmark in prices.columns:
            bn = prices[benchmark].dropna(); bn = bn / bn.iloc[0]
    ...
    with pf_c4:
        ...
        cov_ann = al_lr.cov().values * TDAYS
        pv = float(cur_weights @ cov_ann @ cur_weights)
        pct_rc = (cur_weights * (cov_ann @ cur_weights) / pv
                  if pv > 0 else np.repeat(np.nan, len(cur_weights)))
        fig_rc = go.Figure()
        fig_rc.add_trace(go.Bar(name=_pm("Rủi ro","Risk"), x=asset_cols, y=pct_rc,
            marker_color="#c03030", text=[f"{v:.1%}" for v in pct_rc],
            textposition="outside"))
        fig_rc.add_trace(go.Bar(name=_pm("Tỷ trọng","Weight"), x=asset_cols, y=cur_weights,
            marker_color="#1D9E75", text=[f"{v:.1%}" for v in cur_weights],
            textposition="outside"))
