...
with st.sidebar:
    ...
    mobile_mode = st.toggle(t("mobile_mode"), value=st.session_state.mobile_mode)
    st.session_state.mobile_mode = mobile_mode
    if mobile_mode:
        st.info(t("mobile_info"))

    beginner_default = st.session_state.beginner_mode or mobile_mode
    beginner_mode = st.toggle(t("beginner_mode"), value=beginner_default)
...
with overview_tab:
    responsive_metric_row([
        (t("average_return"),
         ...
         metric_delta_text(metrics_df[t('return_per_year')].mean(), bench_return)),
        (t("average_vol"),
         f"{metrics_df[t('volatility')].mean():.2%}" if not metrics_df.empty else "N/A",
         metric_delta_text(metrics_df[t('volatility')].mean(), bench_vol)),
        (t("avg_sharpe"),
    ...
    if not st.session_state.beginner_mode and not is_mobile():
        ...
        cols_to_show = [t("return_per_year"), t("volatility"), "Sharpe", "Max Drawdown", t("cumulative_profit")]
    ...
    fig_cum.update_layout(
        yaxis_title=t("cumulative_profit"),
        yaxis_tickformat=".0%",
        hovermode="x unified",
        margin=dict(l=10, r=10, t=30, b=10),
        height=chart_height(420, 300),
    )
    ...
    fig_dd_assets.update_layout(
        yaxis_title=t("drawdown_axis"),
        yaxis_tickformat=".0%",
        hovermode="x unified",
        margin=dict(l=10, r=10, t=30, b=10),
        height=chart_height(420, 300),
    ...
    fig_scatter = px.scatter(
        scatter_df, x=t("volatility"), y=t("return_per_year"), text=None if is_mobile() else "Ticker",
...
with verdict_tab:
    ...
    fig_ret_bar.add_trace(go.Bar(
        ...
        marker_color=colors,
        text=None if is_mobile() else [f"{v:.1%}" for v in ret_data.values],
    ...
    fig_mdd_bar.add_trace(go.Bar(
        ...
        marker_color="#c03030",
        text=None if is_mobile() else [f"{v:.1%}" for v in mdd_data.values],
        textposition="outside",
        hovertemplate="%{x}: %{y:.2%}<extra></extra>",
    ))
    ...
    fig_sharpe.add_trace(go.Bar(
        ...
        text=None if is_mobile() else [f"{v:.2f} ({classify_sharpe(v)})" for v in sharpe_data.values],
        textposition="outside",
...
with portfolio_tab:
    ...
    if not st.session_state.beginner_mode and not is_mobile():
        ...
        responsive_metric_row([
            ...
            (t("up_down_capture"), f"{portfolio['up_capture']:.2f} / {portfolio['down_capture']:.2f}"
             if pd.notna(portfolio['up_capture']) and pd.notna(portfolio['down_capture']) else "N/A", None),
        ])
    else:
    ...
    fig_rc_bar.add_trace(go.Bar(name=t("risk_contribution"), x=asset_cols, y=pct_rc,
                                marker_color="#c03030", text=None if is_mobile() else [f"{v:.1%}" for v in pct_rc], textposition="outside"))
    fig_rc_bar.add_trace(go.Bar(name=t("portfolio_weights"), x=asset_cols, y=weights,
                                marker_color="#1D9E75", text=None if is_mobile() else [f"{v:.1%}" for v in weights], textposition="outside"))
...
with optimization_tab:
    ...
    for ticker, r, v in zip(asset_cols, exp_rets, np.sqrt(np.diag(cov))):
        fig_frontier.add_trace(go.Scatter(x=[v], y=[r], mode="markers+text" if not is_mobile() else "markers", text=[ticker],
