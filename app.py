...
LANG = {
    "vi": {
        ...
        "tickers":       "Mã cổ phiếu (dấu phẩy, tối đa 20 mã)",
    ...
    "en": {
        ...
        "tickers":       "Stock tickers (comma-separated, maximum 20 stocks)",
...
with st.sidebar:
    ...
    if st.button(t("reset_w"), use_container_width=True):
...
if not st.session_state.ran:
    ...
    <div style='max-width:680px;margin:48px auto 0;text-align:center'>
      ...
      <p style='opacity:.65;font-size:.92rem'>Nhập mã cổ phiếu ở thanh bên trái và nhấn <b>▶ Phân tích</b> để bắt đầu.</p>
    </div>
    """, unsafe_allow_html=True)
...
with tab_radar:
    ...
    st.dataframe(radar_df, use_container_width=True, height=min(55 + 36*len(asset_cols), 400))
    ...
    st.plotly_chart(fig_cum, use_container_width=True, key="radar_cum_chart")
    ...
    st.plotly_chart(fig_sc, use_container_width=True, key="radar_scatter_chart")
...
with tab_ws:
            ...
            for level, color, dash, lbl in [
                ...
                if pd.notna(level):
                    fig_c.add_hline(y=level, line_dash=dash, line_color=color,
                                    ...
                                    row=1, col=1)
        st.plotly_chart(fig_c, use_container_width=True, key=f"ws_chart_{ws_sel}")
        # RSI sub-chart
        fig_rsi = rsi_chart(wp["price_s"], ws_sel, height=140)
        if fig_rsi.data:
            st.plotly_chart(fig_rsi, use_container_width=True, key=f"ws_rsi_{ws_sel}")
    ...
    if pb:
        ...
        if pb_rows:
            st.dataframe(pd.DataFrame(pb_rows).set_index("Ticker"), use_container_width=True)
...
with tab_pf:
    ...
    for (pid, plbl), pc in zip(presets, pr_cols):
        with pc:
            if st.button(plbl, use_container_width=True, key=f"preset_{pid}"):
        ...
        for idx, tk in enumerate(asset_cols):
            ...
            with wc_cols[idx % len(wc_cols)]:
                raw_w.append(st.number_input(f"{tk} (%)", min_value=0.0, max_value=1000.0,
                                              step=1.0, key=wk))
        apply_btn = st.form_submit_button(t("apply"), type="primary", use_container_width=True)
    ...
    with pf_c1:
        ...
        st.plotly_chart(fig_pf, use_container_width=True, key="pf_wealth_chart")
    with pf_c2:
        ...
        fig_dd.update_layout(yaxis_tickformat=".0%", hovermode="x unified",
            margin=dict(l=10,r=10,t=20,b=10), height=340)
        st.plotly_chart(fig_dd, use_container_width=True, key="pf_dd_chart")
    ...
    with pf_c3:
        ...
        st.plotly_chart(fig_pie, use_container_width=True, key="pf_pie_chart")
    with pf_c4:
        ...
        fig_rc.update_layout(barmode="group", yaxis_tickformat=".0%",
            margin=dict(l=10,r=10,t=20,b=10), height=340)
        st.plotly_chart(fig_rc, use_container_width=True, key="pf_rc_chart")
    ...
    st.plotly_chart(fig_fr, use_container_width=True, key="frontier_chart")
    ...
    st.dataframe(comp_df.style.format({t("return_yr"):"{:.2%}",t("vol_yr"):"{:.2%}",
                                        t("sharpe"):"{:.3f}",t("mdd"):"{:.2%}"}), use_container_width=True)
...
with tab_risk:
    ...
    fig_dd_all.update_layout(yaxis_tickformat=".0%", hovermode="x unified",
        margin=dict(l=10,r=10,t=20,b=10), height=340)
    st.plotly_chart(fig_dd_all, use_container_width=True, key="risk_dd_chart")
    ...
    st.plotly_chart(fig_corr, use_container_width=True, key="risk_corr_chart")
    ...
    if len(asset_cols) >= 2:
        ...
        st.plotly_chart(fig_rc2, use_container_width=True, key=f"risk_rc2_chart_{pa}_{pb}")
    ...
    st.plotly_chart(fig_hist, use_container_width=True, key=f"risk_hist_{dist_sel}")
...
with tab_sys:
    ...
    with sys_screener:
        ...
        with s2:
            ...
            max_dd  = st.number_input("Max drawdown (%)", value=50.0, step=5.0, disabled=not use_dd, key="sc_dd_v")
        ...
        if st.button(t("screener_run"), type="primary"):
            ...
            for col in asset_cols:
                ...
                if use_dd:  checks["Max DD"]      = (pd.notna(row["max_dd"])       and row["max_dd"]       >= -max_dd/100, fmt_pct(row["max_dd"]))
            ...
            if passing:
                ...
                st.dataframe(pass_data.style.format({"ann_ret":"{:.2%}","ann_vol":"{:.2%}","sharpe":"{:.3f}","max_dd":"{:.2%}"}, na_rep="N/A"), use_container_width=True)
                # Add to watchlist
                for col in passing:
    ...
    with sys_watchlist:
        ...
        if wl.empty:
            ...
            st.dataframe(wl.style.format({c:"{:.2%}" for c in ["Return","Volatility","Max DD"] if c in wl.columns} |
                                          {"Sharpe":"{:.3f}","Score":"{:.1f}"} , na_rep="N/A"), use_container_width=True)
            # Scatter chart
            ...
            if not wl_sc.empty:
                ...
                fig_wl.update_layout(xaxis_tickformat=".0%", yaxis_tickformat=".0%",
                    margin=dict(l=10,r=10,t=20,b=10), height=340)
                st.plotly_chart(fig_wl, use_container_width=True, key="wl_scatter_chart")
            # Remove controls
            ...
            for i, tk in enumerate(list(st.session_state.watchlist.keys())):
                with rm_cols[i % len(rm_cols)]:
                    if st.button(f"🗑️ {tk}", key=f"wl_rm_{tk}", use_container_width=True):
    ...
    with sys_data:
        ...
        st.dataframe(prices[asset_cols].tail(20), use_container_width=True)
        st.markdown(f"#### {_pm('Xem trước lợi nhuận hàng ngày','Daily return preview')}")
        st.dataframe(simple_rets[asset_cols].tail(20).style.format("{:.2%}"), use_container_width=True)
