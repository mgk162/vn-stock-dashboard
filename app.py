...
def analyze_wyckoff_by_timeframe(symbol: str, start: date, end: date, source: str, timeframe: str) -> Dict:
    hist, used = _fetch_ohlcv(symbol, start, end, source, timeframe=timeframe)
    fallback_note = _maybe_intraday_fallback_note(used)
    if hist.empty:
        return {"timeframe": timeframe, "phase": _pm("Không có dữ liệu","No data"), "score": np.nan, "setup": "N/A", "source": used, "fallback_note": fallback_note}
    close_s = hist.set_index("date")["close"]
    vol_s = hist.set_index("date")["volume"] if "volume" in hist.columns else pd.Series(dtype=float)
...
def backtest_wyckoff_setups(symbol: str, start: date, end: date, source: str, timeframe: str = "1D", horizon: int = 10) -> Dict:
    hist, used = _fetch_ohlcv(symbol, start, end, source, timeframe=timeframe)
    if hist.empty or len(hist) < 120:
        return {"source": used, "bull_count": 0, "bear_count": 0, "bull_winrate": np.nan, "bear_winrate": np.nan, "bull_avg": np.nan, "bear_avg": np.nan}
...
def workspace_wyckoff_event_markers(symbol: str, start: date, end: date, source: str, timeframe: str = "1D") -> pd.DataFrame:
    hist, used = _fetch_ohlcv(symbol, start, end, source, timeframe=timeframe)
...
def workspace_structure_levels(symbol: str, start: date, end: date, source: str, timeframe: str = "1D") -> Dict:
    hist, used = _fetch_ohlcv(symbol, start, end, source, timeframe=timeframe)
    if hist is None or hist.empty or len(hist) < 30:
        return {"source": used, "range_low": np.nan, "range_high": np.nan, "mid": np.nan, "last_close": np.nan}
...
def price_volume_chart(ticker, start, end, source, timeframe="1D", axis_mode="remove_weekends", height=520, show_ma=True, show_bb=True, show_markers=True, show_trade_range=True, y_scale="linear", marker_density="smart") -> go.Figure:
    hist, source_used = _fetch_ohlcv(ticker, start, end, source, timeframe=timeframe)
    fallback_note = _maybe_intraday_fallback_note(source_used)
    if hist.empty:
        fig = go.Figure()
    ...
    fig.update_layout(
        ...
        title=f"{ticker} · {timeframe}{' · 1D fallback' if '(1D-fb)' in str(source_used) else ''}",
...
def workspace_execution_items(wtr: Dict, mtf_decision: Dict, breadth_status: Dict, pos: Dict, phase4_alerts: List[Dict], decision: Dict | None = None, phase2_metrics: Dict | None = None, stack: Dict | None = None, hold: Dict | None = None) -> List[Tuple[str, bool, str]]:
    ...
    return [
        ...
        (_pm("Timing không mâu thuẫn với verdict","Timing agrees with the verdict"), action_key != "BUY" or timing_label.upper() in {"BUY ZONE", "TEST ENTRY", "BREAKOUT EARLY"}, _pm("BUY chỉ hợp lệ khi timing đã vào vùng hành động","BUY is only valid when timing is actionable")),
...
with tab_ws:
    ...
    mtf_summary = {tf: analyze_wyckoff_by_timeframe(ws_sel, start_date, end_date, data_source, tf) for tf in ["1W", "1D", "30m", "1M"]}
    mtf_decision = phase2_mtf_decision(mtf_summary)
    wy_bt = backtest_wyckoff_setups(ws_sel, start_date, end_date, data_source, timeframe="1D", horizon=10)
    ...
    decision_box["action_key"]   = consistency.get("action_key", decision_box.get("action_key", "WAIT"))
    decision_box["action_label"] = consistency.get("label", decision_box.get("action_label", "N/A"))
    decision_box["tone"]         = consistency.get("tone", decision_box.get("tone", "warn"))
    decision_box["system_status"] = " · ".join([str(x) for x in (consistency.get("why") or [])[:2]]) or decision_box.get("system_status", "")
    decision_box["next_step"]    = consistency.get("next_step", decision_box.get("next_step", ""))
    ...
    ws_markers   = workspace_wyckoff_event_markers(ws_sel, start_date, end_date, data_source, timeframe=ws_tf)
    ws_structure = workspace_structure_levels(ws_sel, start_date, end_date, data_source, timeframe=ws_tf)
