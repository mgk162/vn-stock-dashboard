...
def compute_timing_signals(price_series: pd.Series) -> Dict:
    """Compute MA-based and momentum timing signals for a single price series."""
    p = price_series.dropna()
    if len(p) < 10:
        return {}
    ...
    mom_window = min(63, len(p) - 1)
    momentum = (current_price / p.iloc[-mom_window] - 1) if mom_window > 0 else np.nan

    # Volatility regime: current 20-day vol vs historical average
...
def compute_trade_plan(
    ...
    entry = float(entry_price) if pd.notna(entry_price) and entry_price > 0 else current_price
    ...
    atr_stop = current_price - style_mult * atr if pd.notna(atr) else np.nan
    ...
    stop_candidates = [x for x in [atr_stop, structural_stop, ma_stop] if pd.notna(x) and x < current_price]
    stop_loss = max(stop_candidates) if stop_candidates else current_price * (0.95 if risk_style == "tight" else (0.93 if risk_style == "swing" else 0.91))

    risk_per_share = max(entry - stop_loss, current_price * 0.02)
    ...
    max_trend_score = 0
    for cond in [
        pd.notna(ma20) and current_price > ma20,
        pd.notna(ma50) and current_price > ma50,
        pd.notna(ma200) and current_price > ma200,
        pd.notna(mom_21) and mom_21 > 0,
        pd.notna(mom_63) and mom_63 > 0,
    ]:
        max_trend_score += 1
        trend_score += int(bool(cond))
    trend_pct = trend_score / max_trend_score if max_trend_score else np.nan

    upside_tp2 = tp2 / current_price - 1 if current_price > 0 else np.nan
    downside_sl = stop_loss / current_price - 1 if current_price > 0 else np.nan
    rr_tp2 = (tp2 - current_price) / max(current_price - stop_loss, 1e-9) if current_price > stop_loss else np.nan

    if pd.notna(wy.get("wyckoff_score")) and wy["wyckoff_score"] >= 75 and pd.notna(trend_pct) and trend_pct >= 0.6:
        direction = _pm_lang("Ưu tiên mua theo setup", "Favor entry on setup")
    elif pd.notna(trend_pct) and trend_pct >= 0.55:
...
with st.sidebar:
    ...
    preset = st.selectbox(t("time_range"), list(preset_days.keys()), index=list(preset_days.keys()).index(st.session_state.time_range_preset), key="time_range_preset")
...
with overview_tab:
    ...
    if is_mobile():
        st.subheader(t("cumulative_returns")); st.plotly_chart(fig_cum, use_container_width=True)
        st.subheader(t("drawdown_by_stock")); st.plotly_chart(fig_dd_assets, use_container_width=True)
    else:
        c1, c2 = st.columns(2)
        with c1: st.subheader(t("cumulative_returns")); st.plotly_chart(fig_cum, use_container_width=True)
        with c2: st.subheader(t("drawdown_by_stock")); st.plotly_chart(fig_dd_assets, use_container_width=True)
...
with action_tab:
    ...
    top_ideas = []
    for col in asset_cols:
        timing_pack = compute_timing_signals(prices[col])
...
with verdict_tab:
    ...
    if is_mobile():
        ...
        with c_bar1: st.plotly_chart(fig_ret_bar, use_container_width=True)
        with c_bar2: st.plotly_chart(fig_mdd_bar, use_container_width=True)
...
with portfolio_tab:
    ...
    if is_mobile():
        st.subheader(t("portfolio_vs_benchmark")); st.plotly_chart(fig_pf, use_container_width=True)
        st.subheader(t("portfolio_drawdown_chart")); st.plotly_chart(fig_pdd, use_container_width=True)
        st.subheader(t("portfolio_weights")); st.plotly_chart(fig_pie, use_container_width=True)
        st.subheader(t("risk_contribution")); st.plotly_chart(fig_rc_bar, use_container_width=True)
    else:
        c1, c2 = st.columns(2)
        with c1: st.subheader(t("portfolio_vs_benchmark")); st.plotly_chart(fig_pf, use_container_width=True)
        with c2: st.subheader(t("portfolio_drawdown_chart")); st.plotly_chart(fig_pdd, use_container_width=True)
        c3, c4 = st.columns(2)
        with c3: st.subheader(t("portfolio_weights")); st.plotly_chart(fig_pie, use_container_width=True)
        with c4: st.subheader(t("risk_contribution")); st.plotly_chart(fig_rc_bar, use_container_width=True)
    ...
    if is_mobile():
        ...
        c5, c6 = st.columns(2)
        with c5: st.plotly_chart(fig_rsh, use_container_width=True)
    ...
    if not bench_simple.empty:
        ...
        if is_mobile(): st.plotly_chart(fig_rbeta, use_container_width=True)
        else:
            with c6: st.plotly_chart(fig_rbeta, use_container_width=True)
    else:
        st.info(t("need_benchmark_beta"))
...
with timing_tab:
    ...
    for col in asset_cols:
        sig = compute_timing_signals(prices[col])
        if not sig:
        ...
        with st.expander(f"📈 {t('timing_chart')} — {col}"):
            ...
            fig_ma = go.Figure()
            fig_ma.add_trace(go.Scatter(x=p_series.index, y=p_series, mode="lines", name=col, line=dict(width=1.5), hovertemplate="%{x|%d/%m/%Y}<br>%{y:,.0f}<extra></extra>"))
            if len(p_series) >= 50:
                ma50_series = p_series.rolling(50).mean()
                fig_ma.add_trace(go.Scatter(x=ma50_series.index, y=ma50_series, mode="lines", name="MA50", line=dict(dash="dot", width=1.5, color="#f0a500"), hovertemplate="MA50: %{y:,.0f}<extra></extra>"))
            if len(p_series) >= 200:
                ma200_series = p_series.rolling(200).mean()
                fig_ma.add_trace(go.Scatter(x=ma200_series.index, y=ma200_series, mode="lines", name="MA200", line=dict(dash="dash", width=1.5, color="#c03030"), hovertemplate="MA200: %{y:,.0f}<extra></extra>"))
            fig_ma.update_layout(hovermode="x unified", margin=dict(l=10,r=10,t=30,b=10), height=chart_height(380, 280), yaxis_title=col)
...
with holdings_tab:
    ...
    trade_plan_pm = compute_trade_plan(price_pm, entry_price=pm_entry_price if pm_entry_price > 0 else np.nan, risk_style=risk_style_pm)
