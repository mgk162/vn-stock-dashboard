...
st.markdown(
    ...
    <style>
        ...
        .cmd-card:hover { box-shadow: 0 4px 20px rgba(0,0,0,0.12); }
        .cmd-buy   { background: linear-gradient(135deg,rgba(0,180,100,0.10),rgba(0,180,100,0.04)); border: 1.5px solid rgba(0,180,100,0.35); }
        .cmd-hold  { background: linear-gradient(135deg,rgba(255,160,0,0.10),rgba(255,160,0,0.04));  border: 1.5px solid rgba(255,160,0,0.35); }
        .cmd-avoid { background: linear-gradient(135deg,rgba(220,50,50,0.10),rgba(220,50,50,0.04));   border: 1.5px solid rgba(220,50,50,0.30); }
        .cmd-badge {
            display: inline-block;
            font-size: 1.1rem;
            font-weight: 700;
        ...
        .cmd-badge-buy   { background: rgba(0,180,100,0.15); color: #0a9e60; border: 1px solid rgba(0,180,100,0.4); }
        .cmd-badge-hold  { background: rgba(255,160,0,0.15);  color: #c07800; border: 1px solid rgba(255,160,0,0.4); }
        .cmd-badge-avoid { background: rgba(220,50,50,0.12);   color: #c03030; border: 1px solid rgba(220,50,50,0.3); }
        .cmd-score-bar-wrap { background: rgba(120,120,120,0.12); border-radius: 999px; height: 8px; margin: 6px 0 10px; overflow: hidden; }
        .cmd-score-bar { height: 8px; border-radius: 999px; }
        .cmd-price { font-size: 1.5rem; font-weight: 700; margin: 4px 0; }
        .cmd-meta  { font-size: 0.80rem; opacity: 0.70; }
        .cmd-summary { font-size: 0.85rem; line-height: 1.55; margin-top: 8px; opacity: 0.92; }
        .cmd-next-step {
            background: rgba(0,120,220,0.06);
            border: 1px dashed rgba(0,120,220,0.3);
            border-radius: 10px;
            padding: 7px 12px;
            font-size: 0.80rem;
            margin-top: 8px;
        ...
        .bar-metric-wrap { margin: 4px 0 10px; }
        .bar-metric-label { font-size: 0.78rem; opacity: 0.72; margin-bottom: 3px; }
        .bar-metric-row { display: flex; align-items: center; gap: 8px; }
        .bar-metric-track { flex: 1; background: rgba(120,120,120,0.12); border-radius: 999px; height: 7px; overflow: hidden; }
        .bar-metric-fill  { height: 7px; border-radius: 999px; }
        .bar-metric-val   { font-size: 0.82rem; font-weight: 600; min-width: 48px; text-align: right; }
        ...
        .ind-ob  { background: rgba(220,50,50,0.12);  color:#c03030; border:1px solid rgba(220,50,50,0.3); }
        .ind-os  { background: rgba(0,180,100,0.12); color:#0a9e60; border:1px solid rgba(0,180,100,0.3); }
        .ind-neu { background: rgba(120,120,120,0.10); color:#666;   border:1px solid rgba(120,120,120,0.2); }
        /* ====== MARKET CONTEXT BAR ====== */
        .ctx-bar {
            display: flex;
            gap: 12px;
        ...
        .ctx-item { font-size: 0.80rem; }
        .ctx-label { opacity: 0.65; }
        .ctx-val   { font-weight: 600; margin-left: 4px; }
...
def init_state() -> None:
    defaults = {
        ...
        "_watchlist_json": "{}",
        "analysis_history": [],  # list of {date, tickers, top_action}
    }
    for key, value in defaults.items():
        if key not in st.session_state:
            st.session_state[key] = value
    # Restore watchlist from JSON if watchlist is empty but JSON exists
    if not st.session_state["watchlist"] and st.session_state.get("_watchlist_json", "{}") != "{}":
        try:
            st.session_state["watchlist"] = json.loads(st.session_state["_watchlist_json"])
        except Exception:
            pass
...
def responsive_columns(n_desktop: int, n_mobile: int = 1):
    if is_mobile():
        return [st.container() for _ in range(n_desktop)]
...
# =========================
# Language helper (must be defined before any function that calls it)
# =========================
...
def normalize_price_unit(close_series: pd.Series) -> pd.Series:
    ...
    valid = s.dropna()
    if valid.empty: return s

    median_val = valid.median()
    # VN stock prices in the raw API can arrive as e.g. 130.0 (meaning 130,000 VND).
    # We only multiply by 1000 when the median is clearly in the sub-1000 range
    # AND the 99th-percentile is also below 1000 — this guards against genuine
    # penny-stock prices (e.g. 500–999 VND) being incorrectly scaled.
    # Threshold: median < 500 is safely a "divided by 1000" case; 500-999 is ambiguous
    # so we leave it untouched to avoid 1000x over-inflation.
    if median_val < 500:
        return s * 1000
    return s
...
def liquidity_label(avg_value: float) -> str:
    ...
    if avg_value >= 50_000_000_000:
        return "Excellent Liquidity" if current_lang() == "en" else "Thanh khoản rất tốt"
    if avg_value >= 20_000_000_000:
        return "Good Liquidity" if current_lang() == "en" else "Thanh khoản tốt"
    if avg_value >= 5_000_000_000:
        return "Average Liquidity" if current_lang() == "en" else "Thanh khoản trung bình"
    if avg_value >= 1_000_000_000:
        return "Weak Liquidity" if current_lang() == "en" else "Thanh khoản yếu"
    return "Illiquid" if current_lang() == "en" else "Thanh khoản kém"

def liquidity_pill_class(avg_value: float) -> str:
    label = liquidity_label(avg_value)
    if label in ["Excellent", "Rất tốt", "Good", "Tốt"]:
        return "pill-green"
    if label in ["Moderate", "Trung bình"]:
        return "pill-yellow"
    return "pill-red"
...
def detect_market_regime(price_series: pd.Series) -> Dict:
    ...
    regime_score = np.nan
    if regime == ("Bullish" if current_lang() == "en" else "Tăng giá"):
        regime_score = 85
    elif regime == ("Uptrend but volatile" if current_lang() == "en" else "Xu hướng tăng nhưng biến động"):
        regime_score = 65
    elif regime == ("Sideways / mixed" if current_lang() == "en" else "Đi ngang / lẫn lộn"):
        regime_score = 50
    else:
        regime_score = 25
...
def watchlist_add(ticker: str, snapshot: Dict) -> None:
    ...
    persist_watchlist_to_session(st.session_state.watchlist)
...
def atr_from_close(price_series: pd.Series, window: int = 14) -> float:
    """Approximate ATR using close-to-close True Range (no OHLC available).
    TR = max(|high-low|, |high-prev_close|, |low-prev_close|).
    When only close prices are available we use:
      TR_approx = max(|close - prev_close|, 0)  extended by the rolling std
    to avoid underestimating volatility vs pure diff.
    Specifically: TR = |close - prev_close|, then ATR = EMA(TR, window).
    This is the standard close-only ATR approximation used in practice.
    """
    p = price_series.dropna()
    if len(p) < 3:
        return np.nan
    # True Range approximation with close-only data
    tr = p.diff().abs()
    # Use EMA (exponential smoothing) consistent with Wilder's ATR
    atr_series = tr.ewm(com=window - 1, min_periods=window).mean()
    atr_val = atr_series.iloc[-1] if len(atr_series) >= window else tr.mean()
    return float(atr_val) if pd.notna(atr_val) else np.nan
...
def make_trade_plan_dataframe(trade_plan: Dict, sizing: Dict, holding_pack) -> pd.DataFrame:
    # Guard: holding_pack may be None when no position is held
    hp = holding_pack if isinstance(holding_pack, dict) else {}
    rows = [
        ...
        ["Open PnL %", hp.get("pnl_pct", np.nan)],
        ["Open PnL VND", hp.get("pnl_value", np.nan)],
        ["Protected Stop", hp.get("protected_stop", np.nan)],
        ["R/R From Here", hp.get("rr_from_here", np.nan)],
    ]
...
def compute_rsi(price_series: pd.Series, window: int = 14) -> pd.Series:
    ...
    rsi = 100 - (100 / (1 + rs))
    return rsi.reindex(price_series.index)
...
def rsi_signal(rsi_val: float) -> str:
    if pd.isna(rsi_val):
        return "N/A"
    if rsi_val >= 70:
        return "Overbought" if current_lang() == "en" else "Quá mua"
    if rsi_val <= 30:
        return "Oversold" if current_lang() == "en" else "Quá bán"
    return "Neutral" if current_lang() == "en" else "Trung tính"


def rsi_badge_class(rsi_val: float) -> str:
    if pd.isna(rsi_val):
        return "ind-neu"
    if rsi_val >= 70:
        return "ind-ob"
...
def compute_bollinger(price_series: pd.Series, window: int = 20, n_std: float = 2.0):
    p = price_series.dropna()
    if len(p) < window:
        return pd.Series(dtype=float), pd.Series(dtype=float), pd.Series(dtype=float)
    mid = p.rolling(window).mean()
    std = p.rolling(window).std(ddof=1)
    upper = mid + n_std * std
    lower = mid - n_std * std
    return upper.reindex(price_series.index), mid.reindex(price_series.index), lower.reindex(price_series.index)


def bollinger_position(price: float, upper: float, lower: float, mid: float) -> str:
    if any(pd.isna(x) for x in [price, upper, lower, mid]):
        return "N/A"
    band_width = upper - lower
    if band_width <= 0:
        return "N/A"
    pos_pct = (price - lower) / band_width
    if pos_pct >= 0.85:
        return "Near upper band" if current_lang() == "en" else "Gần band trên"
    if pos_pct <= 0.15:
        return "Near lower band" if current_lang() == "en" else "Gần band dưới"
    return "Mid range" if current_lang() == "en" else "Vùng giữa"


def composite_technical_score(price_series: pd.Series, timing_pack: dict) -> dict:
    """Combine RSI + Bollinger + existing timing signals into one composite 0-100 score."""
    p = price_series.dropna()
    if len(p) < 20:
        return {"composite": np.nan, "rsi": np.nan, "bb_pos": "N/A", "rsi_signal": "N/A"}

    rsi_series = compute_rsi(p)
    rsi_val = float(rsi_series.iloc[-1]) if not rsi_series.empty and pd.notna(rsi_series.iloc[-1]) else np.nan
    bb_upper, bb_mid, bb_lower = compute_bollinger(p)
    last_upper = float(bb_upper.iloc[-1]) if not bb_upper.empty and pd.notna(bb_upper.iloc[-1]) else np.nan
    last_lower = float(bb_lower.iloc[-1]) if not bb_lower.empty and pd.notna(bb_lower.iloc[-1]) else np.nan
    last_mid   = float(bb_mid.iloc[-1])   if not bb_mid.empty   and pd.notna(bb_mid.iloc[-1])   else np.nan
    last_price = float(p.iloc[-1])

    # RSI score: best at 40-60 for uptrend momentum, penalise extremes
    rsi_score = 50.0
    if pd.notna(rsi_val):
        if 45 <= rsi_val <= 65:
            rsi_score = 75.0
            ...
            rsi_score = 45.0  # oversold can be buy opportunity
        elif rsi_val > 72:
            rsi_score = 30.0  # overbought — risky to enter

    # Bollinger position score
    bb_score = 50.0
    if not any(pd.isna(x) for x in [last_upper, last_lower, last_mid, last_price]):
        band_width = last_upper - last_lower
        if band_width > 0:
            pos_pct = (last_price - last_lower) / band_width
            if 0.2 <= pos_pct <= 0.6:
                bb_score = 72.0   # healthy mid range
            elif pos_pct < 0.2:
                bb_score = 60.0   # near support / lower band = opportunity
            elif pos_pct > 0.85:
                bb_score = 28.0   # near upper band = risky entry

    timing_score = timing_pack.get("score", 0)
    timing_max   = timing_pack.get("max_score", 1) or 1
    timing_pct   = clamp(timing_score / timing_max * 100)

    composite = clamp(0.40 * timing_pct + 0.30 * rsi_score + 0.30 * bb_score)

    return {
        "composite": round(composite, 1),
        "rsi": round(rsi_val, 1) if pd.notna(rsi_val) else np.nan,
        "rsi_signal": rsi_signal(rsi_val),
        "rsi_badge": rsi_badge_class(rsi_val),
        "bb_pos": bollinger_position(last_price, last_upper, last_lower, last_mid),
        "bb_upper": last_upper,
        "bb_mid": last_mid,
        "bb_lower": last_lower,
    }
...
def persist_watchlist_to_session(wl_dict: dict) -> None:
    """Encode watchlist into a JSON-serializable string stored in session state."""
    try:
        st.session_state["_watchlist_json"] = json.dumps(
            {k: {ky: (None if (isinstance(vy, float) and np.isnan(vy)) else vy)
                  for ky, vy in v.items()}
             for k, v in wl_dict.items()},
            ensure_ascii=False,
        )
    except Exception:
        pass
...
def restore_watchlist_from_session() -> dict:
    """Restore watchlist from the JSON string in session state (survives reruns)."""
    raw = st.session_state.get("_watchlist_json", "{}")
    try:
...
_is_vi_hdr = current_lang() == "vi"
st.caption(
    t("app_caption") + ("  |  v2.0 — Command Center · RSI · Bollinger · Stress Test · Watchlist Persistence" if not _is_vi_hdr
    else "  |  v2.0 — Trung tâm lệnh · RSI · Bollinger · Stress Test · Watchlist bền vững")
)
...
with st.sidebar:
    ...
    st.markdown("---")
    if st.button(t("analyze"), width="stretch", type="primary"):
        st.session_state.analysis_ran = True
    if st.button(t("reset_weights"), width="stretch"):
...
if not st.session_state.analysis_ran:
    _is_vi_land = current_lang() == "vi"

    st.markdown(f"""
    <div style="max-width:720px;margin:40px auto 0;text-align:center;">
        <div style="font-size:3rem;margin-bottom:12px">📊</div>
        <h2 style="margin:0 0 8px">{"Bắt đầu phân tích danh mục" if _is_vi_land else "Start your portfolio analysis"}</h2>
        <p style="opacity:0.7;font-size:0.95rem">{"Nhập mã cổ phiếu vào thanh bên trái và nhấn Phân tích." if _is_vi_land else "Enter tickers in the sidebar and press Analyse."}</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with s1:
        st.markdown(f"""<div class='section-card' style='text-align:center;'>
            ...
            <b>{"Nhập mã cổ phiếu" if _is_vi_land else "Enter tickers"}</b>
            <p class='small-note'>{"VD: FPT, VCB, HPG (cách nhau bằng dấu phẩy)" if _is_vi_land else "E.g. FPT, VCB, HPG (comma-separated)"}</p>
        </div>""", unsafe_allow_html=True)
    with s2:
        st.markdown(f"""<div class='section-card' style='text-align:center;'>
            <div style='font-size:2rem'>2️⃣</div>
            <b>{"Nhấn ▶ Phân tích" if _is_vi_land else "Press ▶ Analyse"}</b>
            <p class='small-note'>{"Dữ liệu sẽ được tải tự động từ vnstock" if _is_vi_land else "Data is fetched automatically via vnstock"}</p>
        </div>""", unsafe_allow_html=True)
    with s3:
        st.markdown(f"""<div class='section-card' style='text-align:center;'>
            <div style='font-size:2rem'>3️⃣</div>
            <b>{"Xem 🎯 Trung tâm lệnh" if _is_vi_land else "See 🎯 Command Center"}</b>
            <p class='small-note'>{"Quyết định nhanh cho từng cổ phiếu — chỉ trong 10 giây" if _is_vi_land else "Quick decision for each stock — under 10 seconds"}</p>
        </div>""", unsafe_allow_html=True)

    st.markdown(f"<div class='tip-box' style='max-width:720px;margin:16px auto;'>{'📖 <b>Các tab chính:</b> 🎯 Trung tâm lệnh (quyết định nhanh) → 🧠 Hành động đầu tư (phân tích sâu) → ⏱️ Timing (thời điểm vào lệnh) → 📌 Quản trị vị thế (kế hoạch trade) → 💼 Danh mục (phân bổ vốn)' if _is_vi_land else '📖 <b>Main tabs:</b> 🎯 Command Center (quick call) → 🧠 Action (deep analysis) → ⏱️ Timing (entry timing) → 📌 Position Manager (trade plan) → 💼 Portfolio (allocation)'}</div>", unsafe_allow_html=True)
...
data_warnings = []
for col in available_cols:
    total_rows = max(row_counts.get(col, 0), 1)
    ...
    if row_counts.get(col, 0) < 60:
        data_warnings.append(
            f"{col}: {'fewer than 60 trading sessions' if current_lang()=='en' else 'ít hơn 60 phiên giao dịch'}."
        )
    ...
    if last_dates.get(col) is not None and overall_last_date is not None:
        ...
        if gap >= 3:
            data_warnings.append(
                f"{col}: {'data ends' if current_lang()=='en' else 'dữ liệu đến'} "
                f"{last_dates[col].strftime('%Y-%m-%d')}, lag {gap} "
                f"{'days' if current_lang()=='en' else 'ngày'}."
            )
...
for col in asset_cols:
    ...
    metrics_rows.append({
        "Ticker": col,
        t("return_per_year"): ann_ret, "CAGR": cagr, "Expected Return": ann_ret, "Robust Return": robust_ret,
...
# Giữ nguyên dữ liệu, không dùng .dropna() để tránh mất sạch mã mới
aligned_simple = simple_returns[asset_cols]
aligned_log    = log_returns[asset_cols]

# Tính lợi nhuận kỳ vọng độc lập từng mã để không bị lỗi chéo dữ liệu
exp_rets_list = []
for col in asset_cols:
    val = robust_expected_return(aligned_simple[col])
    if pd.isna(val):
        val = aligned_simple[col].mean() * TRADING_DAYS
    exp_rets_list.append(val)
exp_rets = np.array(exp_rets_list, dtype=float)

# Tính ma trận rủi ro (Covariance) với min_periods=30
cov = aligned_log.cov(min_periods=30).values * TRADING_DAYS
cov = np.nan_to_num(cov) # Thay NaN bằng 0 để bộ tối ưu hóa chạy được
...
cmd_center_label   = "🎯 Command Center"  if current_lang() == "en" else "🎯 Trung tâm lệnh"
(cmd_tab, overview_tab, action_tab, holdings_tab, verdict_tab, risk_tab, portfolio_tab,
 optimization_tab, data_tab, screener_tab, timing_tab, watchlist_tab) = st.tabs([
    cmd_center_label, t("overview_tab"), t("action_tab"), holdings_tab_label, t("verdict_tab"), t("risk_tab"),
...
# =================== COMMAND CENTER TAB ===================
with cmd_tab:
    _is_vi = current_lang() == "vi"

    # --- Market context bar ---
    ctx_items = []
    if not pd.isna(bench_return):
        ctx_items.append(("VNINDEX/Benchmark", f"{bench_return:+.1%}/năm" if _is_vi else f"{bench_return:+.1%}/yr"))
    if not pd.isna(bench_vol):
        ctx_items.append(("Biến động TT" if _is_vi else "Mkt vol", f"{bench_vol:.1%}"))
    avg_sharpe_ctx = metrics_df["Sharpe"].mean()
    if not pd.isna(avg_sharpe_ctx):
        ctx_items.append(("Sharpe TB" if _is_vi else "Avg Sharpe", f"{avg_sharpe_ctx:.2f}"))
    if overall_last_date:
        ctx_items.append(("Dữ liệu đến" if _is_vi else "Data as of", overall_last_date.strftime("%d/%m/%Y")))
    ctx_html = "".join([
        f'<span class="ctx-item"><span class="ctx-label">{lbl}</span><span class="ctx-val">{val}</span></span>'
        for lbl, val in ctx_items
    ])
    if ctx_html:
        st.markdown(f'<div class="ctx-bar">{ctx_html}</div>', unsafe_allow_html=True)

    st.markdown(
        f"### {'🎯 Quyết định nhanh — mỗi cổ phiếu, 10 giây' if _is_vi else '🎯 Quick decisions — one card per stock'}"
    )

    # --- One card per stock ---
    n_cols = min(len(asset_cols), 3) if not is_mobile() else 1
    col_groups = [asset_cols[i:i+n_cols] for i in range(0, len(asset_cols), n_cols)]

    for group in col_groups:
        card_cols = st.columns(len(group))
        for cidx, col in enumerate(group):
            with card_cols[cidx]:
                # Gather data
                ann_ret_c   = metrics_df.loc[col, t("return_per_year")]
                ann_vol_c   = metrics_df.loc[col, t("volatility")]
                shp_c       = metrics_df.loc[col, "Sharpe"]
                mdd_c       = metrics_df.loc[col, "Max Drawdown"]
                beta_c      = metrics_df.loc[col, "Beta"]
                alpha_c     = metrics_df.loc[col, "Alpha (year)"]
                last_px     = prices[col].dropna().iloc[-1] if not prices[col].dropna().empty else np.nan
                liq_label   = metrics_df.loc[col, "Liquidity"] if "Liquidity" in metrics_df.columns else "N/A"
                avg_val_20d = metrics_df.loc[col, "Avg Value 20D"] if "Avg Value 20D" in metrics_df.columns else np.nan

                timing_c    = compute_timing_signals(prices[col], volumes[col] if col in volumes.columns else pd.Series(dtype=float))
                tech_c      = composite_technical_score(prices[col], timing_c)
                regime_c    = detect_market_regime(prices[col])
                wy_c        = compute_wyckoff_setup_score(prices[col], volumes[col] if col in volumes.columns else pd.Series(dtype=float))

                action_c = investment_action_engine(
                    ticker=col, ann_ret=ann_ret_c, cagr=metrics_df.loc[col, "CAGR"],
                    ann_vol=ann_vol_c, sharpe=shp_c, mdd=mdd_c, beta=beta_c,
                    alpha=alpha_c, bench_ret=bench_return,
                    avg_value_20d=avg_val_20d,
                    ...
                    timing_overall=timing_c.get("overall"),
                    timing_score=(timing_c.get("score", 0) / timing_c.get("max_score", 1) * 100) if timing_c else np.nan,
                    data_points=int(prices[col].dropna().shape[0]),
                    wyckoff_score=wy_c.get("wyckoff_score"),
                    wyckoff_phase=wy_c.get("phase"),
                    wyckoff_setup=wy_c.get("setup_quality"),
                    robust_return=metrics_df.loc[col, "Robust Return"] if "Robust Return" in metrics_df.columns else np.nan,
                )

                dec_score = action_c["score"]
                action_lbl = action_c["action"]
                conf_lbl   = action_c["confidence"]

                # Determine card class & badge class
                if dec_score >= 68:
                    card_cls  = "cmd-buy"
                    badge_cls = "cmd-badge-buy"
                elif dec_score >= 45:
                    card_cls  = "cmd-hold"
                    badge_cls = "cmd-badge-hold"
                else:
                    card_cls  = "cmd-avoid"
                    badge_cls = "cmd-badge-avoid"

                # Score bar color
                bar_color = "#1D9E75" if dec_score >= 68 else ("#f0a500" if dec_score >= 45 else "#c03030")

                # Build 1-line summary
                summary_parts = []
                if not pd.isna(ann_ret_c):
                    rel = ""
                    if not pd.isna(bench_return):
                        diff = ann_ret_c - bench_return
                        rel = (
                            f" ({'vượt' if _is_vi else 'beats'} {benchmark} {diff:+.1%})" if diff > 0.02
                            else (f" ({'thua' if _is_vi else 'lags'} {benchmark} {diff:+.1%})" if diff < -0.02 else "")
                        )
                    summary_parts.append(f"{'Lợi nhuận' if _is_vi else 'Return'} {ann_ret_c:+.1%}/{'năm' if _is_vi else 'yr'}{rel}.")
                if not pd.isna(shp_c):
                    summary_parts.append(f"Sharpe {shp_c:.2f} ({classify_sharpe(shp_c).lower()}).")
                if not pd.isna(mdd_c):
                    summary_parts.append(f"{'Giảm tệ nhất' if _is_vi else 'Worst drop'} {mdd_c:.0%}.")

                rsi_val_c  = tech_c.get("rsi", np.nan)
                rsi_sig_c  = tech_c.get("rsi_signal", "N/A")
                rsi_bdg_c  = tech_c.get("rsi_badge", "ind-neu")
                bb_pos_c   = tech_c.get("bb_pos", "N/A")
                comp_c     = tech_c.get("composite", np.nan)

                # Market context comparison
                sharpe_vs_mkt = ""
                if not pd.isna(shp_c) and not pd.isna(avg_sharpe_ctx):
                    if shp_c > avg_sharpe_ctx + 0.1:
                        sharpe_vs_mkt = f'<span class="pill pill-green">📈 {"Trên TB thị trường" if _is_vi else "Above mkt avg"}</span>'
                    elif shp_c < avg_sharpe_ctx - 0.1:
                        sharpe_vs_mkt = f'<span class="pill pill-red">📉 {"Dưới TB thị trường" if _is_vi else "Below mkt avg"}</span>'

                # Next-step suggestion
                if dec_score >= 68:
                    next_step = f"→ {'Xem Trade Plan để tính entry/stop/TP' if _is_vi else 'Go to Trade Plan for entry/stop/TP'}"
                elif dec_score >= 45:
                    next_step = f"→ {'Theo dõi Timing để chờ điểm vào tốt hơn' if _is_vi else 'Watch Timing tab for a better entry signal'}"
                else:
                    next_step = f"→ {'Chưa phù hợp — chờ điều kiện tốt hơn' if _is_vi else 'Not ready — wait for better conditions'}"

                liq_pill = (
                    f'<span class="pill pill-{"green" if liq_label in ["Thanh khoản rất tốt", "Thanh khoản tốt", "Excellent Liquidity", "Good Liquidity"] else ("yellow" if liq_label in ["Thanh khoản trung bình", "Average Liquidity"] else "red")}">{escape(str(liq_label))}</span>'
                    if liq_label != "N/A" else ""
)

                price_text = f"{last_px:,.0f} VND" if pd.notna(last_px) else "N/A"
                rsi_text = f"{rsi_val_c:.0f}" if pd.notna(rsi_val_c) else "N/A"

                action_lbl_safe = escape(str(action_lbl))
                ticker_safe = escape(str(col))
                price_text_safe = escape(str(price_text))
                conf_lbl_safe = escape(str(conf_lbl))
                regime_safe = escape(str(regime_c.get("regime", "N/A")))
                rsi_sig_safe = escape(str(rsi_sig_c))
                bb_pos_safe = escape(str(bb_pos_c))
                summary_text_safe = escape(" ".join(summary_parts))
                next_step_safe = escape(str(next_step))

                liq_pill_html = liq_pill if isinstance(liq_pill, str) else ""
                sharpe_vs_mkt_html = sharpe_vs_mkt if isinstance(sharpe_vs_mkt, str) else ""
                comp_html = f"<span class='ind-badge ind-neu'>Comp {comp_c:.0f}/100</span>" if pd.notna(comp_c) else ""

                card_html = f"""
                <div class="cmd-card {card_cls}">
                    <span class="cmd-badge {badge_cls}">{action_lbl_safe}</span>
                    <div class="cmd-price">
                        {ticker_safe} &nbsp;
                        <span style="font-size:0.85rem;font-weight:400;opacity:0.7">{price_text_safe}</span>
                    </div>
                    <div class="cmd-score-bar-wrap">
                        <div class="cmd-score-bar" style="width:{dec_score:.0f}%;background:{bar_color};"></div>
                    </div>
                    <div class="cmd-meta">
                        {'Điểm quyết định' if _is_vi else 'Decision score'}: <b>{dec_score:.0f}/100</b>
                        &nbsp;|&nbsp;
                        {'Tin cậy' if _is_vi else 'Confidence'}: <b>{conf_lbl_safe}</b>
                        &nbsp;|&nbsp;
                        {'Chế độ' if _is_vi else 'Regime'}: <b>{regime_safe}</b>
                    </div>
                    <div style="margin:8px 0 4px">
                        <span class="ind-badge {rsi_bdg_c}">RSI {rsi_text} — {rsi_sig_safe}</span>
                        <span class="ind-badge ind-neu">BB: {bb_pos_safe}</span>
                        {comp_html}
                        {liq_pill_html}
                    </div>
                    <div class="cmd-summary">{summary_text_safe}</div>
                    <div class="cmd-next-step">{next_step_safe}</div>
                </div>
                """
    ...
    st.markdown("---")
    st.markdown(f"### {'📊 Bảng xếp hạng nhanh' if _is_vi else '📊 Quick ranking'}")

    # Build ranking data using pre-computed metrics
    rank_rows = []
    for col in asset_cols:
        ann_ret_r = metrics_df.loc[col, t("return_per_year")]
        ann_vol_r = metrics_df.loc[col, t("volatility")]
        shp_r     = metrics_df.loc[col, "Sharpe"]
        mdd_r     = metrics_df.loc[col, "Max Drawdown"]
        timing_r  = compute_timing_signals(prices[col], volumes[col] if col in volumes.columns else pd.Series(dtype=float))
        wy_r      = compute_wyckoff_setup_score(prices[col], volumes[col] if col in volumes.columns else pd.Series(dtype=float))
        ac_r = investment_action_engine(
            ticker=col, ann_ret=ann_ret_r, cagr=metrics_df.loc[col,"CAGR"],
            ann_vol=ann_vol_r, sharpe=shp_r, mdd=mdd_r,
            beta=metrics_df.loc[col,"Beta"], alpha=metrics_df.loc[col,"Alpha (year)"],
            bench_ret=bench_return,
            avg_value_20d=metrics_df.loc[col,"Avg Value 20D"] if "Avg Value 20D" in metrics_df.columns else np.nan,
            missing_pct=metrics_df.loc[col,"Missing %"] if "Missing %" in metrics_df.columns else np.nan,
            ffill_pct=metrics_df.loc[col,"Forward Fill %"] if "Forward Fill %" in metrics_df.columns else np.nan,
            cvar=metrics_df.loc[col,f"CVaR ({int(alpha_conf*100)}%)"] if f"CVaR ({int(alpha_conf*100)}%)" in metrics_df.columns else np.nan,
            timing_overall=timing_r.get("overall"),
            timing_score=(timing_r.get("score",0)/timing_r.get("max_score",1)*100) if timing_r else np.nan,
            data_points=int(prices[col].dropna().shape[0]),
            wyckoff_score=wy_r.get("wyckoff_score"),
            wyckoff_phase=wy_r.get("phase"),
            wyckoff_setup=wy_r.get("setup_quality"),
            robust_return=metrics_df.loc[col,"Robust Return"] if "Robust Return" in metrics_df.columns else np.nan,
        )
        tech_r = composite_technical_score(prices[col], timing_r)
        rank_rows.append({
            "Ticker": col,
            "Action" if not _is_vi else "Hành động": ac_r["action"],
            "Score": ac_r["score"],
            "Confidence" if not _is_vi else "Tin cậy": ac_r["confidence"],
            "Return/yr" if not _is_vi else "LN/năm": ann_ret_r,
            "Volatility" if not _is_vi else "Biến động": ann_vol_r,
            "Sharpe": shp_r,
            "Max DD": mdd_r,
            "RSI": tech_r.get("rsi", np.nan),
            "Technical" if not _is_vi else "KT tổng hợp": tech_r.get("composite", np.nan),
            "vs Market" if not _is_vi else "vs TT": (ann_ret_r - bench_return if pd.notna(ann_ret_r) and pd.notna(bench_return) else np.nan),
        })

    rank_df = pd.DataFrame(rank_rows).sort_values("Score", ascending=False).set_index("Ticker")
    ret_col_name = "Return/yr" if not _is_vi else "LN/năm"
    vol_col_name = "Volatility" if not _is_vi else "Biến động"
    fmt_rank = {
        ret_col_name: "{:.2%}", vol_col_name: "{:.2%}",
        "Sharpe": "{:.2f}", "Max DD": "{:.2%}",
        "RSI": "{:.1f}", "Technical" if not _is_vi else "KT tổng hợp": "{:.1f}",
        "Score": "{:.1f}",
        "vs Market" if not _is_vi else "vs TT": "{:+.2%}",
    }
    st.dataframe(rank_df.style.format(fmt_rank, na_rep="N/A"), width="stretch")

    # --- How to use guide ---
    st.markdown(
        f"<div class='tip-box'>"
        f"{'💡 <b>Cách dùng trang này:</b> Mỗi card là một cổ phiếu. Màu xanh = ưu tiên mua, vàng = theo dõi, đỏ = tránh. Điểm quyết định tổng hợp nhiều yếu tố: xu hướng, momentum, Sharpe, drawdown, thanh khoản, RSI, Bollinger. <b>Mũi tên → bên dưới card</b> gợi ý bước tiếp theo.' if _is_vi else '💡 <b>How to read this page:</b> Each card represents one stock. Green = consider buying, yellow = watchlist, red = avoid. Decision score combines trend, momentum, Sharpe, drawdown, liquidity, RSI, and Bollinger. <b>The → arrow at the bottom</b> suggests your next step.'}"
        f"</div>",
        unsafe_allow_html=True
    )
...
with overview_tab:
    ...
    if not st.session_state.beginner_mode and not is_mobile():
        ...
        st.dataframe(metrics_df.style.format(fmt_dict), width="stretch")

        metrics_export = metrics_df.reset_index().rename(columns={"index": t("ticker")})

        st.download_button(
            t("download_summary"),
            data=frame_to_csv_bytes(metrics_export, index=False),
            file_name="summary_metrics.csv",
            mime="text/csv",
            key="download_summary_metrics"
        )
    else:
        ...
        st.dataframe(simple_df.style.format({t("return_per_year"): "{:.2%}", "CAGR": "{:.2%}", t("volatility"): "{:.2%}", "Sharpe": "{:.2f}", "Max Drawdown": "{:.2%}", t("cumulative_profit"): "{:.2%}"}), width="stretch")

    # Market context note
    _is_vi_ctx = current_lang() == "vi"
    if not pd.isna(bench_return):
        outperformers = [c for c in asset_cols if not pd.isna(metrics_df.loc[c, t("return_per_year")]) and metrics_df.loc[c, t("return_per_year")] > bench_return + 0.02]
        underperformers = [c for c in asset_cols if not pd.isna(metrics_df.loc[c, t("return_per_year")]) and metrics_df.loc[c, t("return_per_year")] < bench_return - 0.02]
        avg_sharpe_mk = metrics_df["Sharpe"].mean()
        ctx_note_parts = []
        if outperformers:
            ctx_note_parts.append(f"{'✅ Vượt benchmark' if _is_vi_ctx else '✅ Beat benchmark'} ({benchmark} {bench_return:+.1%}): <b>{', '.join(outperformers)}</b>")
        if underperformers:
            ctx_note_parts.append(f"{'⚠️ Thua benchmark' if _is_vi_ctx else '⚠️ Lagging benchmark'}: <b>{', '.join(underperformers)}</b>")
        if not pd.isna(avg_sharpe_mk):
            ctx_note_parts.append(f"{'Sharpe trung bình danh sách' if _is_vi_ctx else 'Avg Sharpe of list'}: <b>{avg_sharpe_mk:.2f}</b> {'(tốt nếu > 0.8)' if _is_vi_ctx else '(good if > 0.8)'}")
        if ctx_note_parts:
            st.markdown("<div class='tip-box'>" + " &nbsp;|&nbsp; ".join(ctx_note_parts) + "</div>", unsafe_allow_html=True)
    ...
    if is_mobile():
        st.subheader(t("cumulative_returns")); st.plotly_chart(fig_cum, width="stretch", key="overview_cumulative_returns_chart")
        st.subheader(t("drawdown_by_stock")); st.plotly_chart(fig_dd_assets, width="stretch", key="overview_drawdown_chart")
    else:
        c1, c2 = st.columns(2)
        with c1: st.subheader(t("cumulative_returns")); st.plotly_chart(fig_cum, width="stretch", key="overview_cumulative_returns_chart")
        with c2: st.subheader(t("drawdown_by_stock")); st.plotly_chart(fig_dd_assets, width="stretch", key="overview_drawdown_chart")
    ...
    st.plotly_chart(fig_scatter, width="stretch", key="overview_risk_return_chart")
    if st.session_state.beginner_mode:
        st.markdown(f"<div class='tip-box'>{t('risk_return_tip')}</div>", unsafe_allow_html=True)


    # Navigation hint
    _is_vi_ov = current_lang() == "vi"
    st.markdown(
        f"<div class='tip-box'>{'→ <b>Bước tiếp theo:</b> Chuyển sang tab <b>🧠 Hành động đầu tư</b> để xem khuyến nghị mua/bán chi tiết cho từng cổ phiếu, hoặc tab <b>🎯 Trung tâm lệnh</b> để có quyết định nhanh.' if _is_vi_ov else '→ <b>Next step:</b> Head to <b>🧠 Action</b> for detailed buy/sell recommendations, or <b>🎯 Command Center</b> for a quick snapshot.'}</div>",
        unsafe_allow_html=True
    )
...
with action_tab:
    ...
    for col in asset_cols:
        ...
        with st.expander(f"📉 {_pm_lang('Biểu đồ giá + volume', 'Price + volume chart')} — {col}"):
            ...
            st.plotly_chart(fig_trade, width="stretch", key=f"action_price_volume_chart_{col}")
            if trade_plan:
    ...
    # Navigation hint
    _is_vi_act = current_lang() == "vi"
    st.markdown(
        f"<div class='tip-box'>{'→ <b>Bước tiếp theo:</b> Vào tab <b>📌 Quản trị vị thế</b> để lập kế hoạch entry/stop/TP chi tiết, hoặc tab <b>⏱️ Timing</b> để xác nhận thời điểm vào lệnh.' if _is_vi_act else '→ <b>Next step:</b> Go to <b>📌 Position Manager</b> for a detailed entry/stop/TP plan, or <b>⏱️ Timing</b> to confirm entry timing.'}</div>",
        unsafe_allow_html=True
    )
...
with verdict_tab:
    ...
    temp_pf = portfolio_metrics(
    aligned_simple,
    aligned_log,
    eq_w_summary,
    rf_annual,
    bench_simple.reindex(aligned_simple.index) if not bench_simple.empty else pd.Series(dtype=float),
    alpha_conf
)
    ...
    if is_mobile():
        st.plotly_chart(fig_ret_bar, width="stretch", key="verdict_return_bar_chart")
        st.plotly_chart(fig_mdd_bar, width="stretch", key="verdict_mdd_bar_chart")
    else:
        c_bar1, c_bar2 = st.columns(2)
        with c_bar1: st.plotly_chart(fig_ret_bar, width="stretch", key="verdict_return_bar_chart")
        with c_bar2: st.plotly_chart(fig_mdd_bar, width="stretch", key="verdict_mdd_bar_chart")
    ...
    st.plotly_chart(fig_sharpe, width="stretch", key="verdict_sharpe_chart")
    ...
    # Navigation hint
    _is_vi_verd = current_lang() == "vi"
    st.markdown(
        f"<div class='tip-box'>{'→ <b>Bước tiếp theo:</b> Tab <b>📉 Rủi ro</b> để xem tương quan và biến động chi tiết, hoặc tab <b>💼 Danh mục</b> để phân bổ vốn.' if _is_vi_verd else '→ <b>Next step:</b> <b>📉 Risk</b> tab for correlations and volatility details, or <b>💼 Portfolio</b> for capital allocation.'}</div>",
        unsafe_allow_html=True
    )
...
with risk_tab:
    ...
    st.plotly_chart(fig_corr, width="stretch", key="risk_corr_matrix_chart")
    if st.session_state.beginner_mode:
        st.markdown(f"<div class='tip-box'>{t('corr_tip')}</div>", unsafe_allow_html=True)

    if not st.session_state.beginner_mode and not is_mobile():
        st.subheader(t("cov_matrix"))
        st.dataframe((log_returns.cov() * TRADING_DAYS).style.format("{:.4f}"), width="stretch")
    else:
        ...
        st.dataframe(risk_simple.style.format({t("volatility"): "{:.2%}", f"VaR {int(alpha_conf*100)}%": "{:.2%}"}), width="stretch")
    ...
    if len(asset_cols) >= 2:
        ...
        st.plotly_chart(fig_rc, width="stretch", key=f"risk_rolling_corr_chart_{a1}_{a2}")
    else:
    ...
    st.plotly_chart(fig_hist, width="stretch", key=f"risk_hist_chart_{dist_asset}")
...
with portfolio_tab:
    ...
    if is_mobile():
        for lbl, key in [(lbl_equal(),"preset_equal"),(lbl_min_risk(),"preset_minvar"),(lbl_best_eff(),"preset_maxsharpe"),(lbl_risk_parity(),"preset_riskparity")]:
            if st.button(lbl, width="stretch", key=key): apply_weight_preset(asset_cols, lbl, cov, exp_rets, rf_annual)
    else:
        pc1, pc2, pc3, pc4 = st.columns(4)
        with pc1:
            if st.button(lbl_equal(), width="stretch", key="preset_equal"): apply_weight_preset(asset_cols, lbl_equal(), cov, exp_rets, rf_annual)
        with pc2:
            if st.button(lbl_min_risk(), width="stretch", key="preset_minvar"): apply_weight_preset(asset_cols, lbl_min_risk(), cov, exp_rets, rf_annual)
        with pc3:
            if st.button(lbl_best_eff(), width="stretch", key="preset_maxsharpe"): apply_weight_preset(asset_cols, lbl_best_eff(), cov, exp_rets, rf_annual)
        with pc4:
            if st.button(lbl_risk_parity(), width="stretch", key="preset_riskparity"): apply_weight_preset(asset_cols, lbl_risk_parity(), cov, exp_rets, rf_annual)
    ...
    with st.form("portfolio_weights_form"):
        ...
        apply_clicked = st.form_submit_button(t("apply"), type="primary", width="stretch")
    ...
    if is_mobile():
        st.subheader(t("portfolio_vs_benchmark")); st.plotly_chart(fig_pf, width="stretch", key="portfolio_vs_benchmark_chart")
        st.subheader(t("portfolio_drawdown_chart")); st.plotly_chart(fig_pdd, width="stretch", key="portfolio_drawdown_chart_main")
        st.subheader(t("portfolio_weights")); st.plotly_chart(fig_pie, width="stretch", key="portfolio_weights_pie_chart")
        st.subheader(t("risk_contribution")); st.plotly_chart(fig_rc_bar, width="stretch", key="portfolio_risk_contribution_chart")
    else:
        c1, c2 = st.columns(2)
        with c1: st.subheader(t("portfolio_vs_benchmark")); st.plotly_chart(fig_pf, width="stretch", key="portfolio_vs_benchmark_chart")
        with c2: st.subheader(t("portfolio_drawdown_chart")); st.plotly_chart(fig_pdd, width="stretch", key="portfolio_drawdown_chart_main")
        c3, c4 = st.columns(2)
        with c3: st.subheader(t("portfolio_weights")); st.plotly_chart(fig_pie, width="stretch", key="portfolio_weights_pie_chart")
        with c4: st.subheader(t("risk_contribution")); st.plotly_chart(fig_rc_bar, width="stretch", key="portfolio_risk_contribution_chart")
    ...
    if is_mobile():
        st.plotly_chart(fig_rsh, width="stretch", key="portfolio_rolling_sharpe_chart")
    else:
        c5, c6 = st.columns(2)
        with c5: st.plotly_chart(fig_rsh, width="stretch", key="portfolio_rolling_sharpe_chart")
    ...
    if not bench_simple.empty:
        ...
        if is_mobile(): st.plotly_chart(fig_rbeta, width="stretch", key="portfolio_rolling_beta_chart")
        else:
            with c6: st.plotly_chart(fig_rbeta, width="stretch", key="portfolio_rolling_beta_chart")
    else:
        st.info(t("need_benchmark_beta"))

    # --- Stress Test ---
    _is_vi_st = current_lang() == "vi"
    st.markdown("---")
    st.subheader("🔥 Stress Test" if not _is_vi_st else "🔥 Kịch bản căng thẳng (Stress Test)")
    st.markdown(
        f"<div class='small-note'>{'Mô phỏng tác động lên danh mục nếu thị trường giảm mạnh, dựa trên Beta và tương quan lịch sử.' if _is_vi_st else 'Simulates portfolio impact under market stress scenarios, based on historical Beta and correlations.'}</div>",
        unsafe_allow_html=True
    )

    scenarios = [
        ("🟡 Điều chỉnh nhẹ (-10%)" if _is_vi_st else "🟡 Mild correction (-10%)", -0.10),
        ("🟠 Điều chỉnh mạnh (-20%)" if _is_vi_st else "🟠 Sharp correction (-20%)", -0.20),
        ("🔴 Sụp đổ (-35%)" if _is_vi_st else "🔴 Crash scenario (-35%)", -0.35),
        ("⚫ Khủng hoảng (-50%)" if _is_vi_st else "⚫ Crisis scenario (-50%)", -0.50),
    ]

    stress_cols = st.columns(len(scenarios))
    port_beta = portfolio.get("beta", np.nan)

    for sc_idx, (sc_label, sc_mkt_drop) in enumerate(scenarios):
        with stress_cols[sc_idx]:
            if pd.notna(port_beta):
                est_drop = float(port_beta) * sc_mkt_drop
                if sc_mkt_drop < 0:
                    est_drop = min(est_drop, sc_mkt_drop * 0.5)

                color = "#c03030" if est_drop < -0.30 else ("#c07800" if est_drop < -0.15 else "#0a9e60")

                st.markdown(
                    f"<div class='section-card' style='text-align:center'>"
                    f"<div style='font-size:0.80rem;opacity:0.7'>{sc_label}</div>"
                    f"<div style='font-size:1.5rem;font-weight:700;color:{color}'>{est_drop:.1%}</div>"
                    f"<div style='font-size:0.75rem;opacity:0.65'>Beta={port_beta:.2f}</div>"
                    f"</div>",
                    unsafe_allow_html=True
                )
            else:
                st.markdown(
                    f"<div class='section-card' style='text-align:center'>"
                    f"<div style='font-size:0.80rem;opacity:0.7'>{sc_label}</div>"
                    f"<div style='opacity:0.5'>N/A</div>"
                    f"</div>",
                    unsafe_allow_html=True
                )
    ...
    portfolio_export = pd.DataFrame({
        ...
        t("sortino"): [portfolio["sortino"]],
        t("portfolio_beta"): [portfolio["beta"]],
        t("portfolio_alpha"): [portfolio["alpha"]],
        t("tracking_error"): [portfolio["tracking_error"]],
        t("info_ratio"): [portfolio["information_ratio"]],
    ...
    st.download_button(
        ...
        mime="text/csv",
        key="download_portfolio_metrics"
    )
...
with optimization_tab:
    ...
    bench_reindexed = bench_simple.reindex(aligned_simple.index) if not bench_simple.empty else pd.Series(dtype=float)
    ...
    st.plotly_chart(fig_frontier, width="stretch", key="optimization_frontier_chart")

    if is_mobile():
        for lbl_w, w_arr in [(t("min_risk_weights"), min_w), (t("best_eff_weights"), tan_w), (t("risk_parity_weights"), rp_w)]:
            st.markdown(f"**{lbl_w}**")
            st.dataframe(pd.DataFrame({t("ticker"): asset_cols, "Weight": w_arr}).set_index(t("ticker")).style.format({"Weight": "{:.2%}"}), width="stretch")
    else:
        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown(f"**{t('min_risk_weights')}**")
            st.dataframe(pd.DataFrame({t("ticker"): asset_cols, "Weight": min_w}).set_index(t("ticker")).style.format({"Weight": "{:.2%}"}), width="stretch")
        with c2:
            st.markdown(f"**{t('best_eff_weights')}**")
            st.dataframe(pd.DataFrame({t("ticker"): asset_cols, "Weight": tan_w}).set_index(t("ticker")).style.format({"Weight": "{:.2%}"}), width="stretch")
        with c3:
            st.markdown(f"**{t('risk_parity_weights')}**")
            st.dataframe(pd.DataFrame({t("ticker"): asset_cols, "Weight": rp_w}).set_index(t("ticker")).style.format({"Weight": "{:.2%}"}), width="stretch")
    ...
    st.dataframe(compare_df.style.format({t("return_per_year"): "{:.2%}", t("volatility"): "{:.2%}", "Sharpe": "{:.3f}", "Max Drawdown": "{:.2%}"}), width="stretch")
    if st.session_state.beginner_mode:
...
with data_tab:
    ...
    st.download_button(
        t("download_diag"),
        data=frame_to_csv_bytes(diagnostics_df, index=False),
        file_name="data_diagnostics.csv",
        mime="text/csv",
        key="download_data_diagnostics"
    )
    st.subheader(t("price_preview"))
    preview_cols = available_cols[:min(5, len(available_cols))]
    st.dataframe(prices_raw[preview_cols].tail(20), width="stretch")
    st.subheader(t("return_preview"))
    st.dataframe(simple_returns[asset_cols].tail(20).style.format("{:.2%}"), width="stretch")
...
with screener_tab:
    ...
    if st.button(t("screener_run"), type="primary"):
        ...
        if not passing:
            ...
            st.plotly_chart(fig_sc, width="stretch", key="screener_summary_chart_plotly")
        ...
        if detail_rows:
            st.dataframe(pd.DataFrame(detail_rows).set_index("Ticker"), width="stretch")
...
with timing_tab:
    ...
    for col in asset_cols:
        ...
        with st.expander(f"📈 {t('timing_chart')} — {col}"):
            ...
            bb_u_t, bb_m_t, bb_l_t = compute_bollinger(prices[col].dropna())
            if not bb_u_t.empty:
                fig_ma.add_trace(go.Scatter(
                    x=bb_u_t.index, y=bb_u_t, mode="lines", name="BB Upper",
                    line=dict(width=1, color="rgba(150,100,200,0.5)", dash="dot"),
                    hovertemplate="%{x|%d/%m/%Y}<br>BB Upper: %{y:,.0f}<extra></extra>"), row=1, col=1)
                fig_ma.add_trace(go.Scatter(
                    x=bb_l_t.index, y=bb_l_t, mode="lines", name="BB Lower",
                    line=dict(width=1, color="rgba(150,100,200,0.5)", dash="dot"),
                    fill="tonexty", fillcolor="rgba(150,100,200,0.04)",
                    hovertemplate="%{x|%d/%m/%Y}<br>BB Lower: %{y:,.0f}<extra></extra>"), row=1, col=1)
            st.plotly_chart(fig_ma, width="stretch", key=f"timing_price_volume_chart_{col}")

            # RSI chart
            rsi_t = compute_rsi(prices[col].dropna())
            if not rsi_t.empty and not rsi_t.dropna().empty:
                rsi_last_t = float(rsi_t.dropna().iloc[-1])
                fig_rsi_t = go.Figure()
                fig_rsi_t.add_trace(go.Scatter(
                    x=rsi_t.index, y=rsi_t, mode="lines", name="RSI(14)",
                    line=dict(color="#6f42c1", width=1.5),
                    hovertemplate="%{x|%d/%m/%Y}<br>RSI: %{y:.1f}<extra></extra>"))
                fig_rsi_t.add_hrect(y0=70, y1=100, fillcolor="rgba(220,50,50,0.06)", line_width=0)
                fig_rsi_t.add_hrect(y0=0, y1=30, fillcolor="rgba(0,180,100,0.06)", line_width=0)
                fig_rsi_t.add_hline(y=70, line_dash="dash", line_color="rgba(220,50,50,0.6)", annotation_text="70 Overbought", annotation_position="top right")
                fig_rsi_t.add_hline(y=30, line_dash="dash", line_color="rgba(0,180,100,0.6)", annotation_text="30 Oversold", annotation_position="bottom right")
                fig_rsi_t.add_hline(y=50, line_dash="dot", line_color="rgba(120,120,120,0.3)")
                fig_rsi_t.update_layout(
                    title=f"RSI (14) — {col}",
                    yaxis=dict(range=[0, 100]),
                    yaxis_title="RSI",
                    hovermode="x unified",
                    margin=dict(l=10, r=10, t=35, b=10),
                    height=chart_height(200, 160),
                    showlegend=False,
                )
                rsi_sig_t = rsi_signal(rsi_last_t)
                rsi_cls_t = rsi_badge_class(rsi_last_t)
                rsi_color_map = {"ind-ob": "#c03030", "ind-os": "#0a9e60", "ind-neu": "#555"}
                st.markdown(
                    f"<div class='tip-box'><span class='ind-badge {rsi_cls_t}'>RSI {rsi_last_t:.1f}</span> "
                    f"<b>{rsi_sig_t}</b> &nbsp;|&nbsp; Bollinger: <b>{compute_bollinger(prices[col].dropna())[0].dropna().iloc[-1]:,.0f} / {compute_bollinger(prices[col].dropna())[2].dropna().iloc[-1]:,.0f}</b> (upper/lower)</div>"
                    if not bb_u_t.dropna().empty and not bb_l_t.dropna().empty else
                    f"<div class='tip-box'><span class='ind-badge {rsi_cls_t}'>RSI {rsi_last_t:.1f}</span> <b>{rsi_sig_t}</b></div>",
                    unsafe_allow_html=True
                )
                st.plotly_chart(fig_rsi_t, width="stretch", key=f"timing_rsi_chart_{col}")
    ...
    if timing_rows:
        st.dataframe(pd.DataFrame(timing_rows).set_index("Ticker"), width="stretch")
...
with holdings_tab:
    ...
    if not trade_plan_pm:
        ...
        st.plotly_chart(fig_pm, width="stretch", key=f"trade_plan_chart_{selected_ticker_pm}")
        ...
        pm_export_display = pm_export.copy()
        pm_export_display["Value"] = pm_export_display["Value"].astype(str)
        st.dataframe(pm_export_display.set_index("Metric"), width="stretch")
        st.download_button(
            ...
            mime="text/csv",
            key=f"download_trade_plan_{selected_ticker_pm}"
        )
...
with watchlist_tab:
    ...
    for idx, col in enumerate(asset_cols):
        with add_cols[idx % len(add_cols)]:
            ...
            if st.button(btn_label, key=f"wl_add_{col}", width="stretch", disabled=already):
                snap = {
                    t("return_per_year"): metrics_df.loc[col, t("return_per_year")],
                    t("volatility"):       metrics_df.loc[col, t("volatility")],
                    "Sharpe":              metrics_df.loc[col, "Sharpe"],
    ...
    if wl_df.empty:
        ...
        for col_name in wl_df.columns:
            if col_name in [t("return_per_year"), t("volatility"), "Max Drawdown", "Alpha (year)", f"VaR ({int(alpha_conf*100)}%)"]:
                ...
                fmt_wl[col_name] = "{:.3f}"
        st.dataframe(wl_df.style.format(fmt_wl, na_rep="N/A"), width="stretch")
        st.download_button(
            "⬇ CSV",
            data=frame_to_csv_bytes(wl_df),
        ...
        if not wl_chart_df.empty:
            ...
            st.plotly_chart(fig_wl, width="stretch", key="watchlist_scatter_chart")
        ...
        for idx, tkr in enumerate(wl_tickers):
            with rm_cols[idx % len(rm_cols)]:
                if st.button(f"🗑️ {tkr}", key=f"wl_rm_{tkr}", width="stretch"):
                    watchlist_remove(tkr)
                    st.rerun()
        if st.button(t("watchlist_clear"), width="stretch"):
